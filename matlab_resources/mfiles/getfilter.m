...
if n<1 | round(n)~=n;
   error('SigSys:generic', 'GETFILTER: n must be a positive integer.'),
end
fs=opt.fs;

if fs>0 & (any(fc>fs/2) | any(fc<=0))
   error('SigSys:generic', 'GETFILTER: fc must be inside the interval [0 fs/2].'),
end
...
if strcmpi(opt.type,'bp') | strcmpi(opt.type,'bs')
    if length(wc)<2,
        warning('SigSys:generic', 'fc must include two frequencies for BP and BS'),
        wc=wc*[0.9,1.1];
    end
    w0=sqrt(wc(1)*wc(2));
    wc=wc(2)-wc(1);
...
elseif strncmpi(opt.alg,'ellip',6)
    error('SigSys:generic', 'Not yet implemented')
else
    error('SigSys:generic', 'Unknown filter algorithm')
end
