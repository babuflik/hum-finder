...
elseif isnumeric(varargin{1})
    R=varargin{1};
    if ~isnumeric(R), error('SigSys:generic', 'R must be numeric'), end
    if size(R,2)~=size(R,3)
        if size(R,3)==1 & size(R,1)==1  % scalar signal, transpose R
            R=R';
        else
            error('SigSys:generic', 'R must have dimension (taumax+1,ny,ny)')
        end
    end
    if nargin>1
        tau=varargin{2};
        if ~isnumeric(tau), error('SigSys:generic', 'tau must be numeric'), end
        if length(tau)~=size(R,1),
            error('SigSys:generic', 'Length of tau must match the size of the first dimension of R')
        end
    end
    if nargin>2
        RMC=varargin{3};
        if ~isempty(RMC)
          if size(RMC,2)~=size(R,1) | size(RMC,3)~=size(R,2) | size(RMC,4)~=size(R,3)
            error('SigSys:generic', 'Last three dimensions of RMC must have the same size as R')
          end
        end
        MC=size(RMC,1);
    end
else
   error('SigSys:generic', 'Incorrect syntax to COVFUN')
end
%     R,tau,fs,name,RMC
...
for k=1:N;
    ...
    elseif isa(m,'sig')
        ...
        error('SigSys:generic', 'Objects must be either SS, SIG or COVFUN')
    end
    ny(k)=size(c);
    if k>1 & ny(k)~=ny(k-1),
        error('SigSys:generic', 'All COVFUN objects to plot must be of same size'),
    end
