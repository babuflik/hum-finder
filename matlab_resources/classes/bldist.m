...
elseif nargin==1 | nargin==2
    if ~isnumericscalar(k) | k<=0 | round(k)~=k | ~isreal(k)
        error('SigSys:generic', 'k must be a positive integer')
    else
        X.k=k;
    end
    if nargin==2
        if ~isnumericscalar(d) | d<=0 | ~isreal(d)
            error('SigSys:generic', 'd must be a positive real number')
        else
            X.d=d;
        end
    else
    ...
    error('SigSys:generic', 'Syntax: bldist(k), bldist(k,d) or bldist')
end
end
