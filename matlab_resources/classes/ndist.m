...
%#
      ...
      elseif nargin==2
        if size(mu,2)>1 | ~isreal(mu)
          error('SigSys:generic', 'mu must be a real column vector')
        end
        n=length(mu);
        for i=1:n
        ...
        if size(P,1)~=n | size(P,2)~=n
          error('SigSys:generic', 'NDIST: P must be square with same dimension as mu')
        end
        if  ~isreal(P)
          error('SigSys:generic', 'NDIST: P must be a real matrix')
        end
        [iscov_ret, iscov_flag] = iscov(P);
        if ~iscov_ret
          switch iscov_flag
            case 1
              error('SigSys:generic', 'NDIST: P must be a real matrix')
            case 2
              error('SigSys:generic', 'NDIST: P must be square')
            case 3
              error('SigSys:generic', 'NDIST: P must be a symmetric matrix')
            case 4
              warning('SigSys:generic', 'NDIST warning: P must be positive semidefinite matrix')
          end
        end
        X.P = P;   %%%
        X.mu = mu; %%%
      else
        error('SigSys:generic', 'Syntax: ndist(mu,P), or just ndist')
      end
    ...
    if ~isempty(X.mu) & length(mu)~=length(X.mu)
        error('SigSys:generic', ['Cannot change the dimensions of mu'])
      end
      X.mu=mu;
    end
    ...
    function X=set.P(X,P)
      if ~isempty(X.P) & length(P)~=length(X.P)
        error('SigSys:generic', ['Cannot change the dimensions of P'])
      end
      if ~iscov(P)
        error('SigSys:generic', ['P is not a valid covariance'])
      end
      X.P=P;
    end
    ...
    function out=fieldwrite(X, arg1,arg2)
      ...
      if ~all(size(eval(X.arg1))==size(arg2))
        error('SigSys:generic', ['Cannot change the dimensions of ',arg1])
      end
      eval([X.arg1,'=arg2;'])
    end
...
%   i is the row index/indices
      if S.type~='()'
        error('SigSys:generic', 'EMPDIST: Use parenthesis for indexing')
      end
      if length(S.subs)>1
         error('SigSys:generic', 'EMPDIST: object is one-dimensional')
      end
      i=S.subs{1};
      if any(i<1) | any(i>length(X.mu))
        error('SigSys:generic', 'Index out of range')
      end
    ...
    function P=var(X)
      ...
      P=X.P;
      if length(P)>1
        error('SigSys:generic', 'X is multi-dimensional, use cov instead')
      end
    end
    ...
    function out=skew(X)
      ...
      if length(X.P)>1
        error('SigSys:generic', 'skew(X) not defined for multi-dimensional variables')
      end
      out=0;
    end
    ...
    function out=kurt(X)
      ...
      if length(X.P)>1
        error('SigSys:generic', 'kurt(X) not defined for multi-dimensional variables')
      end
      out=0;
    end
    ...
    function x=rand(X,n,m)
      ...
      if isempty(n) | ~isreal(n) | n<0 | round(n)~=n
        error('SigSys:generic', 'ndist.rand: n must be a non-negative integer'),
      end
      if length(X.mu)==0
        error('SigSys:generic', 'ndist.rand: X must have a specified mean and variance'),
    ...
    function I=erf(X,x)
      ...
      if nargin<2
        error('SigSys:generic', 'Syntax: erf(X,x)')
      end
      if length(X)>1
        error('SigSys:generic', 'ERF only defined for scalar stochastic variables')
      end
    ...
    function x=erfinv(X,I)
      ...
      %   That is, I(x)=P(X<x)
      if nargin<2
        error('SigSys:generic', 'Syntax: erfinv(X,I)')
      end
      if length(X)>1
        error('SigSys:generic', 'ERF only defined for scalar stochastic variables')
      end
      if I>1 | I<0
        error('SigSys:generic', 'I must be between 0 and 1')
      end
    ...
    function [P,x]=cdf(X,x)
      ...
      if length(X)>1
        error('SigSys:generic', 'CDF only defined for scalar stochastic variables')
      end
      ...
      if nx==1;
        ...
        if nargin<2 | isempty(x)
          ...
          if (size(x,2)~=length(m)) & length(m)~=1
            error('SigSys:generic', 'NDIST.PDF: x must have the same number of columns as length(mu)')
          end
        end
        if isreal(x)
          scale=0.5;
    ...
    function Y=vertcat(X1,varargin)
      ...
      if nargin==1
        Y=X1; %%% copy or not??? /ps
        %error('SigSys:generic', 'NDIST.VERTCAT: at least two input arguments required')
      else
        X2=varargin{1};
        if isa(X2,'ndist')
	  ...
	  Y=X2+X1;   % Use the code in gmdist
        elseif isa(X2,'ndist')
          ...
          if size(mu1,1)~=size(mu2,1)
            error('SigSys:generic', 'X1 and X2 not compatible for PLUS')
          end
          Y=ndist(mu1+mu2,P1+P2);
        else
        ...
        error('SigSys:generic', 'Incorrect argument to PLUS in NDIST class')
      end
    end
    ...
    function Y=times(X1,X2,MC)
      ...
      if isa(X1,'double')  % Y=A.*X
        if size(X1,1)~=size(X2.mu,1);
          error('SigSys:generic', 'X1 and X2 not compatible for TIMES')
        end
    ...
    function Y=mtimes(X1,X2,MC)
      ...
      if isa(X1,'double')  % Y=A*X
        if ~isscalar(X1) & size(X1,2)~=size(X2.mu,1);
          error('SigSys:generic', 'X1 and X2 not compatible for MTIMES')
        end
    ...
    function Y=tt1eval(X,f,varargin)
      ...
      elseif isstr(f)
        try
          ...
          error('SigSys:generic', 'ndist.tt1eval: inline(f) failed')
        end
      else
        error('SigSys:generic', 'ndist.tt1eval: f must be either a file name, an inline object or a string')
      end
    ...
    function Y=tt2eval(X,f,varargin)
      ...
      elseif isstr(f)
        try
          ...
          error('SigSys:generic', 'ndist.tt2eval: inline(f) failed')
        end
      else
        error('SigSys:generic', 'ndist.tt2eval: f must be either a file name, an inline object or a string')
      end
    ...
    function Y=mceval(X,f,varargin)
      ...
      elseif isstr(f)
        try
          ...
          error('SigSys:generic', 'ndist.mceval: inline(f) failed')
        end
      else
        error('SigSys:generic', 'ndist.mceval: f must be either a file name, an inline object or a string')
      end
...
% $$$       end
      ...
      try
        ...
        if size(y,2)~=X.MC
          ...
          try
              ...
              error('SigSys:generic', ['ndist.mceval: function f cannot be evaluated ' ...
                     'for X'])
          end
      end
      Y=empdist(y.');
    ...
    function Y=qteval(X,f,m,varargin)
      ...
      elseif isstr(f)
        try
          ...
          error('SigSys:generic', 'ndist.qteval: inline(f) failed')
        end
      else
        error('SigSys:generic', 'ndist.qteval: f must be either a file name, an inline object or a string')
      end
      ...
      try
        ...
        error('SigSys:generic', 'ndist.qteval: function f cannot be evaluated for X')
      end
      ybar=y*w;
      xbar=x*w;
      nx=1; ny=1; %tmp
    ...
    function [Y,S,fS]=uteval(X,f,type,par,varargin)
      ...
      elseif isstr(f)
        try
          ...
          error('SigSys:generic', 'ndist.uteval: inline(f) failed')
        end
      else
        error('SigSys:generic', 'ndist.uteval: f must be either a file name, an inline object or a string')
      end
      ...
      switch type
        ...
        case  'ct'
          ...
          scaling = n * a^2;
        otherwise
          error('SigSys:generic', ['Type ',type,' of sigma points are not supported. Use ut1, ut2 or ct']) ;
      end
      S = repmat(mu, 1, N);
      ...
      try
        ...
        error('SigSys:generic', 'ndist.uteval: function evaluation f(X,varargin) failed')
      end
      nz = size(fS, 1);
      muz = zeros(nz, 1);
      for i=1:nz
    ...
    function contour(p,ind)
      ...
      nx=length(p);
      if nx==1
        error('SigSys:generic', 'contour requires the s.v. X to be at least two-dimensional')
      end
      if length(ind)~=2;
        error('SigSys:generic', 'index vector ind must be two dimensional')
      end
      %[p,x]=pdf(X(ind));
