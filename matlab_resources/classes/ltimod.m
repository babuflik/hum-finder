...
elseif strcmp(abstype,'loglog')
    ...
    error('SigSys:generic', 'Incorrect plot type')
end
...
for k=1:N;
  ...
  elseif isa(G,'ft')
    ...
    error('SigSys:generic', 'Input arguments must be either LSS, LTF, ARX or FT objects')
  end
  H=G.H; f=G.f; HMC=G.HMC;
  [nf,ny(k),nu(k)]=size(H);
  if k>1 % check dimensions
    if ny(k)~=ny(k-1)
      error('SigSys:generic', 'Objects have different output dimension')
    end
    if nu(k)~=nu(k-1)
      error('SigSys:generic', 'Objects have different input dimension')
    end
  end
...
elseif strncmpi(opt.view,'zp',2)    % Zero-pole
    for k=1:N;
        ...
        elseif isa(G,'lss')
            ...
            error('SigSys:generic', 'Input arguments must be either LSS, LTF or ARX objects')
        end
        ny(k)=size(z,1);
        nu(k)=size(z,3);
        if k>1 % check dimensions
...
elseif strncmpi(opt.view,'rl',2)  % root locus
    for k=1:N;
        ...
        if ~( isa(G,'ltf') | isa(G,'lss')| isa(G,'arx'))
            error('SigSys:generic', 'RLPLOT: Input arguments must be either LSS, LTF  or ARX objects')
        end
        if strcmp(opt.Kmax,'auto')
            ...
            if ~isnumericscalar(Kmax) | Kmax<=0
                error('SigSys:generic', 'RLPLOT: Kmax must be a positive scalar')
            end
        end
        K=0:Kmax/1000:Kmax;
        G=lss(G);
