...
elseif nargin==3
    ...
    if size(Y,1)~=length(f)
        error('SigSys:generic', 'TF constructor: number of rows in E must equal the length of f')
    end
    if size(Y,2)~=length(t)
        error('SigSys:generic', 'TF constructor: number of columns in E must equal the length of t')
    end
    fs=2; MC=0; name='';
    xlabel=[]; ulabel=[]; ylabel=[]; tlabel=[]; desc=[]; marker=[]; markerlabel=[];
else
    error('SigSys:generic', 'Incorrect syntax to TFD constructor')
end
...
error('SigSys:generic', 'Cannot change protected fields in TFD object.')
end
