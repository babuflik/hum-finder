...
function P = pdfclass
    ...
    function Y=horzcat(varargin)
      %HORZCAT
      error('SigSys:generic', 'PDFCLASS: Stochastic vectors are column vectors by convention. Use vertcat, [X1;X2], instead')
    end %%%
    ...
    function I=erf(X,x)
      ...
      if nargin<2
        error('SigSys:generic', 'PDFCLASS.ERF: Syntax erf(X,x)')
      end
      [P,xg]=cdf(X);
    ...
    function x=erfinv(X,I)
      ...
      %   That is, I(x)=P(X<x)
      if nargin<2
        error('SigSys:generic', 'PDFCLASS.ERFINV: Syntax erfinv(X,I)')
      end
      if I>1 | I<0
        error('SigSys:generic', 'PDFCLASS.ERFINV: I must be between 0 and 1')
      end
      [P,xg]=cdf(X);
    ...
    function [P,x]=cdf(X,x)
      ...
      if isempty(x)
        ...
        %      error('SigSys:generic', 'PDFCLASS.CDF: Specified x value outside observation range in CDF')
        %   end
        ind1=find(x<xsort(1));
    ...
    function Y=mldivide(A,X)  %%% OBS! kommer inte funkar!!!! /ps
      ...
      if ~isa(A,'double')  % Y=A*X
        error('SigSys:generic', 'First argument must be numeric to MLDIVIDE')
      end
      if size(A,1)~=length(X)
        error('SigSys:generic', 'Incorrect size of A in AY=X (size(A,1)=length(X) required)')
      end
      MC=max([100,X.MC]);
      x=rand(X,MC);
      for k=1:MC
    ...
    function Y=fusion(X1,X2)
      ...
      if ~ (isa(X1,'ndist') & isa(X2,'ndist'))
        error('SigSys:generic', 'pdfclass.fusion only implemented for the normal distribution')
      end
      if length(X1)~=length(X2)
        error('SigSys:generic', 'pdfclass.fusion: Length of X1 and X2 must be the same')
      end
      I1=pinv(X1.P);
      I2=pinv(X2.P);
      P=pinv(I1+I2);
    ...
    function Y=safefusion(X1,X2)
      ...
      if ~ (isa(X1,'ndist') & isa(X2,'ndist'))
        error('SigSys:generic', 'pdfclass.safefusion only implemented for the normal distribution')
      end
      if length(X1)~=length(X2)
        error('SigSys:generic', 'pdfclass.safefusion: Length of X1 and X2 must be the same')
      end
      I1=pinv(X1.P);
      I2=pinv(X2.P);
      % Step 1
    ...
    function plot(varargin)
      ...
      if length(varargin{1})>1
        error('SigSys:generic', 'PDFCLASS.PLOT: Specify the dimension of the stochastic vector for plot by using the call plot(X(dim))')
      end
    ...
    function cdfplot(varargin)
      ...
      %   plot calls pdfplot
      if length(varargin{1})>1
        error('SigSys:generic', 'PDFCLASS.CDFPLOT: Specify the dimension of the stochast vector for plot by using the call plot(X(dim))')
      end
    ...
    function erfplot(varargin)
      ...
      %   plot calls pdfplot
      if length(varargin{1})>1
        error('SigSys:generic', 'PDFCLASS.ERFPLOT: Specify the dimension of the stochast vector for plot by using the call plot(X(dim))')
      end
    ...
    function varargout=plot2(varargin)
      ...
      if nargin==0
        error('SigSys:generic', 'PDFCLASS.PLOT2: No inputs given')
      end
      ...
      while k<length(varargin)
        ...
        elseif isvector(varargin{k})   % Data vector
          ...
          error('SigSys:generic', 'PDFCLASS.PLOT2: Unknown syntax: pdf or property value pairs expected')
        end
      end
      if length(dim)~=2;
        error('SigSys:generic', 'PDFCLASS.PLOT2: Dimension dim must be two-dimensional')
      end
opt=struct('conf',[90],'view','pdf','axis',0,'col','bgrmyk','fontsize',[],'linewidth',[],'Xlim',[],'Ylim',[],'legend','auto','markersize',10);
      ...
      for k=1:N;
        ...
        leg{k}=symbolic(X);
        if length(X)==1
          error('SigSys:generic', 'PDFCLASS.PLOT2 applies only to PDFCLASS objects representing stochastic vectors, not stochastic numbers')
        end
        if max(dim)>length(X) | any(round(dim)~=dim) | min(dim)<0
          error('SigSys:generic', 'PDFCLASS.PLOT2: Incorrect dimension in dim')
        end
        if isa(X,'empdist')
	  ...
	  hh(k)=plot(mu(1),mu(2),['+' opt.col(kcol)],'parent',opt.axis);
	    ...
	    l=erfinv(expdist(1),c/100);
            plot(mu(1)+l*Ph(1,:)*[cos(phi);sin(phi)],...
             ...
             %error('SigSys:generic', ['PDFCLASS.PLOT2 does not support pdf of class ',class(X)])
          x=rand(X,X.MC);
          p=pdf(X,x);
          %          size(p),size(x)
    ...
    function surf(X,dim,varargin)
      ...
      if nargin==0
        error('SigSys:generic', 'PDFCLASS.SURF: No inputs given')
      end

      if nargin<2, dim=[1 2]; end
      if length(dim)~=2;
        error('SigSys:generic', 'PDFCLASS.SURF: Dimension dim must be two-dimensional')
      end
      if isstr(dim)
        error('SigSys:generic', 'PDFCLASS.SURF: Second argument is the dimension [i,j], not a string')
      end
      ...
      if length(X)==1 | ~isa(X,'pdfclass')
        error('SigSys:generic', 'PDFCLASS.SURF: applies only for PDFCLASS objects for stochastic vectors, not stochastic numbers')
      end
      if max(dim)>length(X) | any(round(dim)~=dim) | min(dim)<0
        error('SigSys:generic', 'PDFCLASS.SURF: Incorrect dimension in dim')
      end
    ...
    function pdfplot(varargin)
      ...
      if nargin==0
        error('SigSys:generic', 'PDFCLASS.PLOT: No inputs given')
      end
      ...
      while k<length(varargin)
        ...
        elseif isstr(varargin{k})
          ...
          error('SigSys:generic', 'PDFCLASS.PLOT: Unknown syntax: pdf or property value pairs expected')
        end
      end
      ...
      for k=1:N;
        dist=M{k};
        if length(dist)>1
          error('SigSys:generic', 'pdfclass.plot cannot plot multivariate distributions') %XXX
        end
        leg{k}=symbolic(dist);
        kcol=rem(k-1,length(opt.col))+1; %XXX
