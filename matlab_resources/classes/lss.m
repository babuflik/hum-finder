...
if nargin<1
%    error('SigSys:generic', 'LSS contructor expects an input argument: see help lss')
elseif isa(varargin{1},'lss')|isa(varargin{1},'nl')|isa(varargin{1},'ltf')||isa(varargin{1},'arx')
    ...
    elseif isa(s,'nl')  % NL object
        if nargin<2
           error('SigSys:generic', 'Conversion from NL to LSS requires a linearization point')
        end
    ...
    if ~isstr(varargin{1})
        if nargin==1;
            ...
            if ~isnumericscalar(fs) & ~isnan(fs)
                error('SigSys:generic', 'LSS constructor: fs expected as second input argument')
            end
            if fs==0 | isinf(fs)
               fs=NaN;
            elseif fs<0
               error('SigSys:generic', 'LSS constructor: fs>0')
            end
        end
        nn=varargin{1};
        if ~isnumeric(nn) | length(nn)~=4
            error('SigSys:generic', 'LSS constructor: Structure index must be of length(4), nn=[nx nu nv ny]')
        else
            nx=nn(1); nu=nn(2); nv=nn(3);  ny=nn(4);
            if nv>0; ne=ny; end;
        end
        if ny==0
            error('SigSys:generic', 'LSS constructor: ny>0 required')
        end
        if any(nn<0)
            error('SigSys:generic', 'LSS constructor: orders nn must be positive')
        end
        if any(round(nn)~=nn)
            error('SigSys:generic', 'LSS constructor: orders nn must be integers')
        end
%        error('SigSys:generic', 'LSS constructor: String input required when single argument used to LSS')
    elseif isstr(varargin{1})
        ...
        elseif strcmpi(varargin{1},'int')
            ...
            error('SigSys:generic', 'Unknown string option to LSS')
        end
        if nargin>1
            error('SigSys:generic', sprintf('''%s'' may not be followed by any arguments.', ...
                  varargin{1}))
        end
...
if nuz~=nu
   error('SigSys:generic', 'LSS.KALMAN: Number of inputs nu must be the same in model and signal')
end
...
if opt.alg==1;  % Stationary KF
  ...
  [yhat,xhat]=ssdlsim(A-A*K*C,[K B-A*K*D],C,[zeros(ny,ny) D],[y.', zeros(ny, 1); u.', zeros(nu, 1)],xhat);
...
if isempty(A) & isempty(B) & isempty(C) & isempty(D)
    warning('SigSys:generic', ['Unspecified LSS model with ',num2str(nx),' states, ',...
          num2str(nu),' inputs, ',...
