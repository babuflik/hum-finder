...
elseif nargin==1
    if ~isnumericscalar(d) | d<=0 | round(d)~=d | ~isreal(d)
        error('SigSys:generic', 'd must be a positive integer')
    else
	X.d=d;
    end
    X.state=round(1e6*rand); % Set unique random state
else
    error('SigSys:generic', 'Syntax: chi2dist(d) or chi2dist')
end
end
...
if d<=0,
   ...
   if nargout<2,warning('SigSys:generic', msg), end
end
Y=chi2dist(d);
end
...
if nargin<2, error('SigSys:generic', 'CHI2DIST.DETECT: a second data argument y required'), end

if isa(y,'sig'); y=y.y; end
if length(y)>1; error('SigSys:generic', 'CHI2DIST.DETECT: y must be a scalar '), end
h=erfinv(Z,1-pfa);
b=(y>h);
level=erf(Z,y);
end
