...
elseif isa(varargin{1},'lss')  % Convert from SIG object
    ...
    if nargin<2
        error('SigSys:generic', 'Input to FT must either be a SIG object, or Y,f (and YMC)')
    end
    Y=varargin{1};
    f=varargin{2};
    if length(f)~=size(Y,1)
        error('SigSys:generic', 'Length of f and number of rows in Y must be the same')
    end
    if nargin>2;
        YMC=varargin{3};
        MC=size(YMC,1);
        if MC>0 & (size(Y,1)~=size(YMC,2) | size(Y,2)~=size(YMC,3))
            error('SigSys:generic', 'Last two dimensions of YMC must be the same as the dimensions of Y')
        end
    end
end
...
if any(freqind>length(f))
    error('SigSys:generic', 'Requested index larger than length of frequency vector')
end
if any(freqind<1)
    error('SigSys:generic', 'Frequency index must be positive')
end
if any(freqind~=round(freqind))
    error('SigSys:generic', 'Frequency index must be integers')
end
if nargin>1
  yind=varargin{2};
  if isstr(yind)
    if strcmp(yind,':')
       ...
       error('SigSys:generic', 'Signal index must be a vector or :')
    end
  end
else
  yind=1:ny;
...
if any(yind>ny)
     error('SigSys:generic', 'Signal index larger than the number of signals')
end
if any(yind<1)
    error('SigSys:generic', 'Signal index must be positive integers')
end
if any(yind~=round(yind))
    error('SigSys:generic', 'Signal index must be integers')
end
...
if any(diff(diff(Y.f)~=0))
   error('SigSys:generic', 'FT.IFT: frequency grid must be uniform')
end
fmax=max(Y.f);
fs=2*fmax;
...
elseif isnumeric(Y2)    % op on a vector
    if size(Y1.Y)==size(Y2)
        ...
        error('SigSys:generic', ['Using ',upper(op),' on a vector/matrix and a FT object, size(Y.Y) must coincide with the size of the vector/matrix'])
   end
elseif isa(Y2,'ft') % op on two signals
    if size(Y1.Y)==size(Y2.Y) & size(Y1.f)==size(Y2.f)
        ...
        elseif Y2.MC==0 & ~isempty(Y1.YMC);
            ...
            error('SigSys:generic', 'FT: Check that the MC number coincides with the actual number of MC simulations in the YMC field')
        end
    else
        error('SigSys:generic', ['Using ',upper(op),'on two FT objects, N1==N2, fs1==fs2, f1==f2'])
   end
else
   error('SigSys:generic', ['Inappropriate argument for ',upper(op),' in FT objects'])
end
end
...
while k<length(varargin)
    ...
    elseif isstr(varargin{k})
            ...
            error('SigSys:generic', 'Inputs must be a FT object, or property value pairs')
    end
end
