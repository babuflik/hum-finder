...
elseif nargin==1
    ...
    if ~isnumeric(data)
        error('SigSys:generic', 'Data must be numeric')
    end
    X.x=data;
    X.MC=size(data,1);
else
    error('SigSys:generic', 'Data must be specified')
end
end
...
if strcmp(arg1,'x')
   error('SigSys:generic', ['Cannot change the empirical data in EMP'])
else
...
if ~isa(X,'empdist')
    error('SigSys:generic', 'X must be an empirical distribution')
end
if isstr(pdflist)
    pdflist={pdflist};
end
...
for k=1:n;
   Y=eval(pdflist{k});
   if ~isa(Y,'pdfclass')
       error('SigSys:generic', [pdflist{k},' is not a valid PDF'])
   end
   Yhat{k}=estimate(Y,X);
...
if length(X)>1
    error('SigSys:generic', 'PDF only defined for scalar stochastic variables')
end
[N,n]=size(X.x);
xmin=min(X.x);
xmax=max(X.x);
...
if isa(X1,'double')  % Y=A+X
   ...
   elseif size(X1,1)==nx & size(X1,2)==1
      ...
      error('SigSys:generic', 'For X1 being numeric, it must be a column vector of same size as X2 or a scalar')
   end
elseif isa(X2,'double')  % Y=Xa
   ...
   elseif size(X2,1)==nx & size(X2,2)==1
      ...
      error('SigSys:generic', 'For X2 being numeric, it must be a column vector of same size as X1 or a scalar')
   end
else
...
if size(A,1)~=length(X)
   error('SigSys:generic', 'Incorrect size of A in AY=X')
end
x=rand(X,MC,1);
for k=1:MC                              %
    y(:,k)=A\x(:,k);
