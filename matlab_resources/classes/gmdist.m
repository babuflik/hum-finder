...
elseif nargin==1
   if isa(varargin{1},'gmdist') % Copy PDF with new state
        ...
        if isnumericscalar(k) & k>0 & round(k)==k
            ...
            error('SigSys:generic', 'GMDIST usage: gmdist(k) requires k to be a positive constant')
        end
    end
elseif nargin==3
   alpha=varargin{1};
   ...
   k=size(alpha,1);
   if ~isvector(alpha)
      error('SigSys:generic', 'GMDIST: alpha must be a vector')
   elseif abs(sum(alpha)-1)>1e-15
     sum(alpha)-1
      error('SigSys:generic', 'GMDIST: alpha must be a vector that sums to one')
   end
   if size(mu,2)~=k | ~isreal(mu)
      error('SigSys:generic', 'GMDIST: mu must be a real matrix with size(mu,2)==length(alpha)')
   end
   n=size(mu,1);
   if ndims(P)==2
      if size(P,2)~=k | ~isreal(P)
         error('SigSys:generic', 'GMDIST: P must be a real matrix with third dimension equal to length(alpha)')
      else
         PP(1,1,:)=P; P=PP;
      end
   elseif  ~isreal(P)
      error('SigSys:generic', 'GMDIST: P must be a real matrix')
   elseif size(P,3)~=k
      error('SigSys:generic', 'GMDIST: last dimension of P must be equal to length(alpha)')
   end
   if size(P,1)~=n | size(P,2)~=n
      error('SigSys:generic', 'GMDIST: first two dimensions of P must be equal to size(mu,1)')
   end
   for m=1:k
      if ~iscov(P(:,:,m))
         error('SigSys:generic', 'GMDIST: P(:,:,m) must be a symmetric positive definite matrix for each m')
      end
   end
else
    error('SigSys:generic', 'GMDIST usage: gmdist(alpha,mu,P), gmdist(k) or gmdist')
end
X.alpha=alpha;
X.mu=mu;
X.P=P;
...
%   i is the row index/indices
      if S.type~='()'
        error('SigSys:generic', 'NDIST: Use parenthesis for indexing')
      end
      if length(S.subs)>1
         error('SigSys:generic', 'NDIST: object is one-dimensional')
      end
      i=S.subs{1};
...
elseif strncmpi(type,'m',1)
   ...
   error('SigSys:generic', ['gmdist.map: unrecognized mode option: ',type])
end
end
...
randn('state',X.state);
if nargin<3,  m=1; end
if nargin<2,  error('SigSys:generic', 'No dimension specified'), end
if length(X)>1 & m>1
    error('SigSys:generic', 'For multivariate GM distributions X, number of columns is the length of X and cannot be set')
end
A=cumsum(X.alpha);
for j=1:n*m
    u=rand;
...
if isempty(x)
   ...
   if any(x<xsort(1)) | any(x>xsort(end))
      error('SigSys:generic', 'Specified x value outside observation range in CDF')
   end
...
if nx==1; % somewhat optimized code
    if nargin<2 | isempty(x)
        ...
        if size(x,2)~=1
            error('SigSys:generic', 'x must be a column vector for univariate X')
        end
    end
    N=length(x);
    v=squeeze(X.P(1,1,:));
...
elseif nx>1
    if nargin>1
        [N,m]=size(x);
        if nx~=m
            error('SigSys:generic', 'Number of columns in x and rows in mu must be the same')
        end
    elseif nx==2
       ...
       error('SigSys:generic', 'gmdist.pdf: Autogrid yet only for 1D and 2D s.v. use pdf(X,x)')
    end
    if isreal(x)
        k=0.5;
    else
...
elseif isa(X2,'pdfclass')
   ...
   error('SigSys:generic', 'Incorrect argument to PLUS in GMDIST class')
end
end
