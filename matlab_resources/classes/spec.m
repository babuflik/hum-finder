...
elseif isnumeric(varargin{1})
    Phi=varargin{1};
    if ~isnumeric(Phi), error('SigSys:generic', 'Phi must be numeric'), end
    if size(Phi,2)~=size(Phi,3)
        error('SigSys:generic', 'Phi must have dimension (nf,ny,ny)')
    end
    if nargin>1
        f=varargin{2};
        if ~isnumeric(f), error('SigSys:generic', 'Frequency vector f must be numeric'), end
        if length(f)~=size(Phi,1),
            error('SigSys:generic', 'Length of f must match the size of the first dimension of Phi')
        end
    end
    if nargin>2
        PhiMC=varargin{3};
        ...
        if ~isempty(PhiMC) & (size(PhiMC,2)~=size(Phi,1) | size(PhiMC,3)~=size(Phi,2) | size(PhiMC,4)~=size(Phi,3))
            error('SigSys:generic', 'Last three dimensions of PhiMC must have size as Phi')
        end
    else
        PhiMC=[];
    end
else
   error('SigSys:generic', 'Incorrect syntax to SPEC')
end
...
for k=1:N;
    ...
    ny(k)=size(m.Phi,2);
    if k>1 & ny(k)~=ny(k-1)
        error('SigSys:generic', 'ny must be the same for all spectra')
    end
    for mm=1:ny(k)
        for nn=1:mm
            if ny(k)>1
