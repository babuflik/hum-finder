...
if nargin<1
    error('SigSys:generic', 'LTF contructor expects at least one input argument: see help ltf')
...
elseif isstr(varargin{1}) % Special LTF
    ...
    elseif strcmpi(varargin{1},'z')
        ...
        error('SigSys:generic', 'LTF: Unknown string option')
    end
    if nargin>1
...
elseif nargin==1 & isvector(varargin{1}) % empty structure
    ...
    if ~isnumeric(nn) | size(nn,1)>1
       error('SigSys:generic', 'LTF constructor: nn must be a numeric row vector in a call ltf(nn)')
    end
    na=nn(1);
    ...
    if length(nn)>5
      error('SigSys:generic', 'LTF constructor: Too long [na,nb,nk,nu,ny] vector.')
    end
    nn=[na nb nk nu ny];
    a=[]; b=[];
elseif nargin==3 | nargin==2;
    ...
    if ~isnumeric(a) | ~isnumeric(b)
      error('SigSys:generic', 'LTF constructor: a and b must be numeric in ltf(b,a).')
    end
    if isempty(a) | isempty(b)
      error('SigSys:generic', 'LTF constructor: a and b cannot be empty in ltf(b,a).')
    end
    ...
    if ~isnumericscalar(fs) | fs<=0
        error('SigSys:generic', 'LTF constructor: fs must be positive or NaN')
    end
    ny=size(b,1);
    nb=size(b,2);
    nu=size(b,3);
    ...
    if isempty(indb)
        %error('SigSys:generic', 'All parameters in b are zero')
        b=zeros(ny,1,nu);
        indb=inda;
    else
        b=b(:,indb(1):indb(end),:);
    ...
    error('SigSys:generic', 'LTF constructor: Incorrect number of inputs')
end
...
if isnumeric(arg2) | isa(arg2,'cell')
   if strcmp(arg1,'b')
      if isa(arg2,'cell')
         error('SigSys:generic', 'b cannot be assiged a cell')
      end
      if isequal(size(arg2),size(b))
         b=arg2;
         %eval([arg1,'=',mat2str(arg2),';'])
      else
         error('SigSys:generic', 'Cannot change size of b in LTF objects')
      end
   elseif strcmp(arg1,'a')
      ...
      if isequal(size(arg2),size(a))
         ...
         error('SigSys:generic', 'Cannot change size of a in LTF objects')
      end
   elseif strcmp(arg1,'sysMC')
      ...
      elseif ~isa(arg2{1},'ltf')
         error('SigSys:generic', 'sysMC must be a cell array of LTF objects')
      elseif isequal(size(arg2{1}.nn),nn)  % Check sysMC argument
         sysMC=arg2;
      else
         sysMC=arg2;
         %error('SigSys:generic', 'Argument of sysMC does not match the LTF dimensions')
      end
   else
      error('SigSys:generic', ['There is no visible field called ' arg1 '.'])
   end
else
   error('SigSys:generic', 'Field value must be numeric, cell or PDFCLASS.')
end
end %function
...
if nargin<2
   error('SigSys:generic', 'LTF.ARRAYREAD: both input indeces in G(j,i) are required')
end
nk=nn(3);
nu=nn(4);
ny=nn(5);
if isstr(ii)
    if strcmp(ii,':')
       ...
       error('SigSys:generic', 'LTF.ARRAYREAD: Inputs ii must be a vector or :')
    end
end
if isstr(jj)
    if strcmp(jj,':')
       ...
       error('SigSys:generic', 'LTF.ARRAYREAD: Outputs jj must be a vector or :')
    end
end
if any(ii<=0) | any(ii~=round(ii))
     error('SigSys:generic', 'LTF.ARRAYREAD: Input (column) index must be positive integers')
end
if any(jj<=0) | any(jj~=round(jj))
     error('SigSys:generic', 'LTF.ARRAYREAD: Output (column) index must be positive integers')
end
if any(ii>nu)
     error('SigSys:generic', 'LTF.ARRAYREAD: Input (column) index larger than the number of inputs')
end
if any(jj>ny)
...
elseif ny<=nu
    ...
    error('SigSys:generic', 'ny<=nu is required for right inverse')
end
end %function
...
if all(s1.nn==s2.nn)
    if size(b1)==size(b2) & size(a1)==size(a2)
        ...
        warning('SigSys:generic', 'Warning: cancellation might occur, try minreal first')
    end
end
end %function
...
if isa(s1,'ltf') | isa(s2,'ltf')
    ...
    elseif isa(s1,'ltf')
        ...
        error('SigSys:generic', 's1 must be either a LTF object or a matrix')
    end
    if isnumeric(s2)
    ...
    elseif isa(s2,'ltf')
        ...
        error('SigSys:generic', 's2 must be either a LTF object or a matrix')
    end
    if ny1~=ny2 | nu1~=nu2
       disp(['nu1=',num2str(nu1),', ny1=',num2str(ny1),', nu2=',num2str(nu2),', ny2=',num2str(ny2)])
       error('SigSys:generic', 'Incorrect dimensions in s1+s2. nu1=nu2, ny1=ny2 required.')
    end
    for i=1:nu1;
        for j=1:ny1;
...
if isa(s1,'ltf') | isa(s2,'ltf')
    ...
    if ny2~=nu1
       ...
       error('SigSys:generic', 'Incorrect dimensions in s1*s2.')
    end
...
if isa(s1,'ltf') | isa(s2,'ltf')
    ...
    if nu1~=nu2
       error('SigSys:generic', ['LTF: Incorrect dimensions in s1.*s2, nu1=nu2 is required but nu1=',num2str(nu1),' and nu2=',num2str(nu2)])
    end
    if ny1~=ny2
       error('SigSys:generic', ['LTF: Incorrect dimensions in s1.*s2, ny1=ny2 is required but ny1=',num2str(ny1),' and ny2=',num2str(ny2)])
    end
...
if isa(s1,'ltf') | isa(s2,'ltf')
    ...
    if  ny2>1
        error('SigSys:generic', 'Currently, only scalar feedback path (s2*s1 scalar) is implemented for LTF')
    end
    if ny1~=nu2
       disp(['ny1=nu2 and ny2=nu1 are required, '])
       disp(['but nu1=',num2str(nu1),' nu2=',num2str(nu2),' ny1=',num2str(ny1),' and ny2=',num2str(ny2)])
       error('SigSys:generic', 'Incorrect dimensions in feedback(s1,s2).')
    end
    g0=s2*s1;
...
elseif nargin<2;
    ...
    if isa(s1,'ltf') | isa(s2,'ltf')
        ...
        elseif isa(s1,'ltf')
            ...
            error('SigSys:generic', 'Input must be either a LTF object or a matrix')
        end
...
if s.nn(3)<0
    error('SigSys:generic', 'LTF.LTF2LSS: Only causal LTF can be converted to SS')
end
...
switch lower(form)
  ...
  otherwise
    error('SigSys:generic', ['Illegal form: ' form ])
end
sys=lss(A,B,C,D,fs);
...
if s.fs>0
    error('SigSys:generic', 'LTF object already in discrete time')
end
T=1/fs;
...
elseif strcmp(method(1:3),'foh');
    ...
    error('SigSys:generic', ['LTF.C2D: Method ',method,' unknown'])
end
...
if isnan(s.fs)
    error('SigSys:generic', 'LTF object already in continuous time')
end
...
elseif strcmp(method(1:3),'foh');
    ...
    error('SigSys:generic', ['Method ',method,' unknown'])
end
...
if ~isnumeric(MC) | length(MC)>1 | MC<0 | MC~=round(MC)
    error('SigSys:generic', 'LTF.UNCERTAIN: MC must be a positive integer')
elseif MC==0
   Gu=G;
   return
end
...
if isa(X,'pdfclass')
   ...
   if N==length(X)
      ...
      error('SigSys:generic', 'LTF.UNCERTAIN: when setting a number of coefficients to a PDFCLASS, the length of X must match')
   end
else
   error('SigSys:generic', 'LTF.UNCERTAIN: X must be a PDFCLASS object')
end
end %function
...
if nu~=nusig
    error('SigSys:generic', 'LTF.estimate: The system and signal must have the same number of inputs')
end
if ny~=nysig
    error('SigSys:generic', 'LTF.estimate: The system and signal must have the same number of outputs')
end
if ~isempty(z.yMC)
   MC=length(z.yMC);
else
...
G=varargin{1};
if G.nn(3)<0
   error('SigSys:generic', 'LTF.IMPULSE only accepts causal systems')
end
if nargout>0
...
G=varargin{1};
if G.nn(3)<0
   error('SigSys:generic', 'LTF.STEP only accepts causal systems')
end
if nargout>0
...
[b,a,Gfs]=unpack(G);
if isempty(a)
   error('SigSys:generic', 'LTF.FILTER: The LTF object is empty')
end
if nu~=nuG;
   error('SigSys:generic', 'LTF.FILTER: The number of inputs in G (nu) and number of outputs in u (ny) must be equal')
end
if isinf(fs)
    error('SigSys:generic', 'LTF.FILTER works only for discrete time models, use simulate otherwise')
end
if isinf(Gfs)
    error('SigSys:generic', 'LTF.FILTER works only for discrete time signals, use simulate otherwise')
end
...
if nu~=nuG;
   error('SigSys:generic', 'The number of inputs in G (nu) and number of outputs in u (ny) must be equal')
end
if isinf(fs)
    error('SigSys:generic', 'LTF filtfilt method works only for discrete time models, use simulate otherwise')
end
if isinf(Gfs)
    error('SigSys:generic', 'LTF filtfilt method works only for discrete time signals, use simulate otherwise')
end
...
if isinf(fs)
    error('SigSys:generic', 'LTF ncfilter method works only for discrete time models, use simulate otherwise')
end
if isinf(Gfs)
    error('SigSys:generic', 'LTF ncfilter method works only for discrete time signals, use simulate otherwise')
end
...
if nargin<2;
    error('SigSys:generic', 'LTF.SIMULATE: An input must be specified')
end
if ~isa(u,'sig'),
    error('SigSys:generic', 'LTF.SIMULATE: The input u must be a SIG object')
end
...
[b,a,fs]=unpack(G);
if a(1)==0
   error('SigSys:generic', 'LTF.SIMULATE: non-causal system (a(1)~=0) cannot be simulated')
end
...
if fs>0
    ...
    %-------------------------------
    if isnan(u.fs)
        error('SigSys:generic', 'LTF.SIMULATE: Signal must be discrete time when model is discrete time')
    end
    umat=u.y;
    [N,nudata]=size(umat);
    if N<nudata; error('SigSys:generic', 'LTF.SIMULATE: Input matrix u should have more rows than columns'), end
    if nudata~=nu
       error('SigSys:generic', 'LTF.SIMULATE: Number of inputs in u not compatible with model')
    end
    ...
    %-------------------------------
    if ~isnan(u.fs)
        error('SigSys:generic', 'LTF.SIMULATE: Signal must be continuous time when model is continuous time')
    end
    umat=u.y;
    [N,nudata]=size(umat);
    if nudata~=nu
        error('SigSys:generic', 'LTF.SIMULATE: Number of inputs in SIG object not compatible with model')
    end
...
elseif isa(s1,'pdfclass');  % Uncertain static ltf
   ...
   error('SigSys:generic', 'LTF: Unsupported object type')
end
...
elseif isnan(s1.fs) | isnan(s2.fs)
    error('SigSys:generic', 'LTF: cannot mix continuous and discrete time models')
elseif s1.fs==s2.fs
    fs=s1.fs;
else
    error('SigSys:generic', 'LTF: Multi-rate LTI operations are not possible')
end
