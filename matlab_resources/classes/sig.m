...
if nargin<1
    ...
    %error('SigSys:generic', 'SIG constructor requires at least one input argument.')
end
y=varargin{1};
if isa(y,'struct')
   try
     ...
     error('SigSys:generic', 'SIG constructor: the input is a struct that does not have the required fields y')
   end
...
if isa(y,'sig')
    %error('SigSys:generic', 'SIG constructor input is already a SIG object')
...
%    error('SigSys:generic', 'SIG: Data y contains NaN or Inf')
end
if  ~isnumeric(y) % | isempty(y) allow empty y from 061214
    error('SigSys:generic', 'SIG: y must be a numeric vector or matrix.')
end
...
if nargin>1
    if isnumericscalar(varargin{2})
	   ...
	   fs=varargin{2};
           if fs<=0
               error('SigSys:generic', 'SIG constructor: fs must be positive')
           end
           t=(0:N-1)'/fs;
        else
           t=varargin{2};
    ...
    elseif isvector(varargin{2});
        ...
        if length(t)~=size(y,1)
            error('SigSys:generic', 'SIG constructor: Length of t must equal number of rows in y')
        end
        fs=NaN;
    else
        error('SigSys:generic', 'SIG constructor: Incorrect second input to SIG (t or fs expected)')
    end
else
    fs=1;
    t=(0:N-1)';
...
if nargin>2
    ...
    if size(u,1)~=N
       error('SigSys:generic', 'SIG constructor: size(u,1) must equal size(y,1)=N')
    end
else
       u=zeros(N,0);
end
if nargin>3
    ...
    if size(x,1)~=N
       error('SigSys:generic', 'SIG constructor: size(x,1) must equal size(y,1)=N')
    end
else
    x=zeros(N,0);
end
if nargin>4 & (size(varargin{5},2)~=ny  | size(varargin{5},1)~=N )
    ...
    if ~isempty(yMC) & size(yMC,2)~=N
       error('SigSys:generic', 'SIG constructor: size(yMC,2) must equal size(y,1)=N')
    end
    if ~isempty(yMC) & size(yMC,3)~=size(y,2)
       error('SigSys:generic', 'SIG constructor: size(yMC,3) must equal size(y,2)=ny')
    end
    Py=[];
elseif nargin>4 & size(varargin{5},2)==ny
    ...
    if ~isempty(Py)
       if size(Py,1)~=N
          error('SigSys:generic', 'SIG constructor: size(Py,1) must equal size(y,1)=N')
       end
       if size(Py,3)~=size(Py,2)
          error('SigSys:generic', 'SIG constructor: Py(i,:,:) must be square')
       end
       if ~iscov(squeeze(Py(1,:,:)))
          error('SigSys:generic', 'SIG constructor: Py(i,:,:) must be covariance matrices')
       end
    end
    yMC=[];
else
...
if nargin>5 & size(varargin{6},2)~=size(varargin{6},3)
    ...
    if ~isempty(xMC) & size(xMC,1)~=MC
        error('SigSys:generic', 'SIG constructor: The number of Monte Carlo samples in xMC and yMC should be the same')
    end
    if ~isempty(xMC) & size(xMC,2)~=N
       error('SigSys:generic', 'SIG constructor: size(xMC,2) must equal size(y,1)=N')
    end
    if ~isempty(xMC) & size(xMC,3)~=size(x,2)
       error('SigSys:generic', 'SIG constructor: size(xMC,3) must equal size(x,2)=nx')
    end
else
    xMC=[];
end
if nargin>5 & size(varargin{6},2)==size(varargin{6},3)
    ...
    if ~isempty(Px)
       if size(Px,1)~=N
          error('SigSys:generic', 'SIG constructor: size(Px,1) must equal size(x,1)=N')
       end
       if size(Px,3)~=size(Px,2)
          error('SigSys:generic', 'SIG constructor: Px(i,:,:) must be square')
       end
       if ~iscov(squeeze(Px(1,:,:)))
          error('SigSys:generic', 'SIG constructor: Px(i,:,:) must be covariance matrices')
       end
    end
else
    Px=[];
...
function s=set.y(s,y)
   if ~isnumeric(y)
      error('SigSys:generic', 'SIG: y must be numeric.')
   end
   if isempty(s.y) | isequal(size(s.y),size(y)) | isnan(s.fs)
      s.y=y;
   else
     error('SigSys:generic', 'SIG: Cannot change size of y by assignment')
   end
end
function s=set.u(s,u)
   if ~isnumeric(u)
      error('SigSys:generic', 'SIG: u must be numeric.')
   end
   if isempty(s.u) | isequal(size(s.u),size(u))
      s.u=u;
   else
     error('SigSys:generic', 'SIG: Cannot change size of u by assignment')
   end
end

function s=set.x(s,x)
   if ~isnumeric(x)
      error('SigSys:generic', 'SIG: x must be numeric.')
   end
   if isempty(s.x) | isequal(size(s.x),size(x))
      s.x=x;
   else
     error('SigSys:generic', 'SIG: Cannot change size of x by assignment')
   end
end

function s=set.yMC(s,yMC)
   if ~isnumeric(yMC)
      error('SigSys:generic', 'SIG: yMC must be numeric.')
   end
   if isempty(s.yMC) | isequal(size(s.yMC),size(yMC))
      s.yMC=yMC;
   else
     error('SigSys:generic', 'SIG: Cannot change size of yMC by assignment')
   end
end

function s=set.xMC(s,xMC)
   if ~isnumeric(xMC)
      error('SigSys:generic', 'SIG: xMC must be numeric.')
   end
   if isempty(s.xMC) | isequal(size(s.xMC),size(xMC))
      s.xMC=xMC;
   else
     error('SigSys:generic', 'SIG: Cannot change size of xMC by assignment')
   end
end

function s=set.t(s,t)
   if ~isnumeric(t)
      error('SigSys:generic', 'SIG: t must be numeric.')
   end
   if ~isreal(t)
      error('SigSys:generic', 'SIG: t must be real')
   end
   [n,m]=size(t);
   if n>1 & m>1
      error('SigSys:generic', 'SIG: t must be a vector')
   end
   t=t(:);
   if length(t)~=size(s.y,1)
      error('SigSys:generic', 'SIG: t must be of the same length as y')
   end
   if any(diff(t)<0)
%      error('SigSys:generic', 'SIG: t must be non-decreasing')
   end
   s.t=t;
   Ts=diff(t);
...
function s=set.fs(s,fs)
   if ~isnumeric(fs)
      error('SigSys:generic', 'SIG: fs must be numeric.')
   end
   if isnumericscalar(fs) & fs>0
      s.fs=fs;
%      warning('SigSys:generic', 'SIG warning: cannot change time indeces to match this fs, change t recommended')
   elseif isnumericscalar(fs) & fs==0
      s.fs=NaN;
   elseif isnan(fs)
      s.fs=NaN;
   else
      error('SigSys:generic', 'SIG: Sampling frequency must be positive')
   end
end
...
function s=set.xlabel(s,xlabel)
   ...
   elseif isempty(xlabel)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field xlabel must be a cell of length nx, [], or string if nx=1'])
   end
end
function s=set.ylabel(s,ylabel)
   ...
   elseif isempty(ylabel)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field ylabel must be a cell of length ny, [], or string if ny=1'])
   end
end
...
function s=set.ulabel(s,ulabel)
   ...
   elseif isempty(ulabel)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field ulabel must be a cell of length nu, [], or string if nu=1'])
   end
end
...
function s=set.tlabel(s,tlabel)
   if isempty(tlabel) | isstr(tlabel)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field tlabel must be a string'])
   end
end
...
function s=set.name(s,name)
   if isempty(name) | isstr(name)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field name must be a string'])
   end
end
...
function s=set.desc(s,desc)
   if isempty(desc) |isstr(desc)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field desc must be a string'])
   end
end
...
function s=set.moviefile(s,moviefile)
   if isempty(moviefile) | isstr(moviefile)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field moviefile must be a string'])
   end
end
...
function s=set.markerlabel(s,markerlabel)
   if isempty(markerlabel) | isstr(markerlabel)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field markerlabel must be a string'])
   end
end
...
function s=set.marker(s,marker)
   if isempty(marker) | isvector(marker)
      ...
      error('SigSys:generic', ['SIG fieldwrite: Field marker must be a vector'])
   end
end
...
case '.'
     zout =s.(S.subs);
case '{}'
     error('SigSys:generic', 'SIG: sig arrays not defined')
case '()'
     tind=S.subs{1};
     if length(S.subs)<3
         j=':';
     ...
     if isstr(tind)
         if strcmp(tind,':')
            ...
            error('SigSys:generic', 'Inputs to arrayread must be a vector or :')
         end
     end
     if isstr(i)
         if strcmp(i,':')
     ...
     if any(i>ny)
          error('SigSys:generic', 'Output (row) index larger than the number of outputs')
     end
     if any(j>nu)
          error('SigSys:generic', 'Input (column) index larger than the number of inputs')
     end
     if any(tind<1) | any(tind>N)
         error('SigSys:generic', 'Specified time interval out of range')
     end
     yy=s.y(tind,i);
     tt=s.t(tind);
     if ~isempty(s.u)
...
if size(z1,1)~=size(z2,1)
   error('SigSys:generic', 'The outputs must be of the same length')
elseif ~isequal(z1.u,z2.u)
   error('SigSys:generic', 'The inputs must be the same')
elseif ~isequalwithequalnans(z1.fs,z2.fs)
   error('SigSys:generic', 'The sampling rates must be the same')
else
  z = struct('u', z1.u,...
             'x', [z1.x, z2.x],...
             'y', [z1.y, z2.y],...
...
if size(z1,3)~=size(z2,3)
   error('SigSys:generic', 'The inputs must be of the same dimension')
elseif size(z1,2)~=size(z2,2)
   error('SigSys:generic', 'The outputs must be of the same dimension')
elseif size(z1,4)~=size(z2,4)
   error('SigSys:generic', 'The states must be of the same dimension')
elseif ~isequalwithequalnans(z1.fs,z2.fs)
   error('SigSys:generic', 'The sampling rates must be the same')
else
...
if size(z1,1)~=size(z2,1)
   error('SigSys:generic', 'The outputs must be of the same length')
elseif isequal(z1.u,z2.u)
   error('SigSys:generic', 'The inputs must be different, otherwise use vertcat')
elseif ~isequalwithequalnans(z1.fs,z2.fs)
   error('SigSys:generic', 'The sampling rates must be the same')
else
  z = struct('u', [z1.u, z2.u],...
...
if isempty(z.moviefile)
   error('SigSys:generic', 'sig.play: empty field fieldname')
end
if ~exist(z.moviefile)==2
   error('SigSys:generic', ['sig.play: file ',z.moviefile,' not found'])
end
if ispc
   p=which(z.moviefile);
   if isempty(p)
     error('SigSys:generic', 'SIG.PLAY: animation file not found on path')
   end
   dos(['start ',p])
else
   error('SigSys:generic', 'SIG.PLAY: animations can only be started in Windows')
end
...
for k=1:length(varargin)
    if isa(varargin{k},'sig')
        y=varargin{k};
        if isempty(y.u)
            error('SigSys:generic', 'No input in the SIG object to plot in UPLOT')
        end
        u=sig(y.u,y.t,[]);
        u.ylabel=y.ulabel;
        u.tlabel=y.tlabel;
...
for k=1:length(varargin)
    if isa(varargin{k},'sig')
        y=varargin{k};
        if isempty(y.x)
            error('SigSys:generic', 'No states in the SIG object to plot in XPLOT')
        end
        %        x=x2y(y);
...
if ~isnumericscalar(opt.tlabel)
   error('SigSys:generic', 'sig.xplot2: tlabel must be a real scalar')
end
    ...
    if isa(varargin{k},'sig')
        ...
        elseif strcmp(opt.view,'plot3')
	       plot3(y.x(:,i),y.x(:,j),zeros(size(y.x,1),1),[col(kk),'.-'],'parent',opt.axis,'linewidth',opt.linewidth)
            ...
            error('SigSys:generic', ['sig.plot: unknown option for view: ',opt.view])
        end
        hold(opt.axis,'on')
        if opt.tlabel>0
...
elseif isnumeric(z2)    % op on a vector
    if size(z1.y)==size(z2)
        ...
        error('SigSys:generic', ['Using ',upper(op),' on a vector/matrix and a SIG object, size(z.y) must coincide with the size of the vector/matrix'])
   end
...
%        z.Py=z.Py+z2.Py;
        ...
        elseif z2.MC==0 && ~isempty(z1.yMC);
            ...
            error('SigSys:generic', 'SIG: Check that the MC number coincides with the actual number of MC simulations in the yMC field')
        end
    else
        if size(z1.y,1)~=size(z2.y,1)
           error('SigSys:generic', ['SIG: Using ',upper(op),' on two SIG objects, assure that the lengths are equal (N1=',num2str(size(z1.y,1)),', N2=',num2str(size(z2.y,1)),').'])
        elseif size(z1.y,2)~=size(z2.y,2)
           error('SigSys:generic', ['SIG: Using ',upper(op),' on two SIG objects, assure that the signal dimensions are equal (ny1=',num2str(size(z1.y,2)),', ny2=',num2str(size(z2.y,2)),').'])
        elseif size(z1.fs)~=size(z2.fs)
           error('SigSys:generic', ['SIG: Using ',upper(op),' on two SIG objects, assure that the sampling frequencies are equal (fs1=',num2str(z1.fs),', fs2=',num2str(z2.fs),').'])
        else
           error('SigSys:generic', ['SIG: Using ',upper(op),' on two SIG objects, assure that the time vectors are equal.'])
        end
    end
elseif isa(z2,'pdfclass') % op on a sig and a noise distribution
   ...
   ny2=length(z2);
   if ny2~=ny
      error('SigSys:generic', 'Signal dimension ny is not equal to the noise dimension length(z2)')
   end
   e=rand(z2,N);
   z.y=feval(op,z.y,e);
   if z.MC==0;
   ...
   error('SigSys:generic', ['Inappropriate argument for ',upper(op),' in SIG objects'])
end
end
...
if nargin<2; n=1; end
if isempty(z.yMC)
   error('SigSys:generic', 'No MC data in input argument')
end
MC=size(z.yMC,1);
if isnan(z.fs)
  in2=z.t;
...
if ~isnumericscalar(fs)
   error('SigSys:generic', 'fs must be a numeric scalar')
end
...
if ~isnumeric(t2)
   error('SigSys:generic', 't2 must be numeric')
end
...
%if isempty(ind), t1,t2(i), end
  ...
  elseif strcmpi(opt.method,'spline')
    y2=interp1(t1,y1,t2,'spline','extrap');
     ...
     error('SigSys:generic', ['Unknown method ',method])
  end
  wip(:,k)=y2;
end
y=wip(:,1:ny);
...
if ~isnumericscalar(n) | ~isnumericscalar(m) | round(n)~=n | round(m)~=m | m<1 | n<1
   error('SigSys:generic', 'n and m must be positive integers')
end
y=z1.y;
fs=z1.fs;
if isnan(fs)
   error('SigSys:generic', 'RESAMPLE applies only to discrete time signals with Uniform sampling')
end
T=1/fs;
if isempty(z1.t)
   t=(1:length(y))/fs;
...
if Nf/2~=round(Nf/2)
   Nf=Nf+1;
   warning('SigSys:generic', ['Warning: Nf must be even, and is changed to ',num2str(Nf)])
end
if ~isnan(fs)  % DTFT
    if isempty(opt.f)
        for k=1:ny
...
[N,ny]=size(y);
%if N<ny, warning('SigSys:generic', 'Warning: N<ny'), end
taumax=opt.taumax;
...
for j=1:ny
    for i=j:ny
        ...
        elseif strncmpi(opt.method,'freq',2) % frequency domain
            ...
            error('SigSys:generic', 'method is one of ''direct'', ''conv'' or ''freq''')
        end
        Rtot(:,i,j)=R;
        Rtot(:,j,i)=R;
    end
...
if isnan(fs)
   if opt.method==2 | opt.method==3
      error('SigSys:generic', 'sig.sig2spec: Welch and Blackman-Tukey are not available for non-uniform data')
   else
      fs=1/z.t(end);
   end
end
...
if ischar(method)
    ...
    else error('SigSys:generic', [method,' is not a valid method'])
    end
elseif method>0 & method<6
    m=method;
else
    error('SigSys:generic', [method,' is not a valid method'])
end
if m==2
...
fs=z.fs;
if isnan(fs);
    error('SigSys:generic', 'TFD can only be computed for discrete time signals')
end
N=length(y);
...
while k<length(varargin)
    ...
    if isstr(varargin{k})  % Property Value Pair
	   ...
	   error('SigSys:generic', 'Non-sig object arguments must appear in property-value pairs')
        end
    else   % vector or struct
        N=N+1;
        Z{N}=varargin{k};
...
for k=1:N;
  ...
  z=Z{k};
  if ~isa(z,'sig')
     error('SigSys:generic', 'SIG: input to plot functions must all be SIG objects')
  end
    z=z(:,opt.ind); %xxx
  ...
  if k>1 % check dimensions
    if ny(k)~=ny(k-1)
      error('SigSys:generic', 'Signals have different output dimension')
    end
    if nu(k)~=nu(k-1)
      %error('SigSys:generic', 'Signals have different input dimension')
      % Not important
    end
  end
  if ny(k)>8
    error('SigSys:generic', 'sig.plot can only plot up to eight subplot rows, try to reduce signal dimension for y with plot(y(:,1:8)) or similar')
  end
  if nu(k)>8
    error('SigSys:generic', 'sig.plot can only plot up to eight subplot columns, try to reduce signal dimension for u with plot(y(:,:,1:8)) or similar')
  end
