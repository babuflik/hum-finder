classdef nl
%NL is the model object for Non-Linear time-varying systems
%
%   help nl.nl    gives help for the constructor
%   methods nl    lists all methods for this class
...
function m=nl(f,h,nn,fs)
%NL constructor non-linear dynamic models
%   m=nl(f,h,nn,fs)
...
%   f and h are inline objects or strings with arguments t,x,u,th,
...
%   is equivalent to
%     f=inline('-th(1)*x^2-th(2)*x','t','x','u','th');
%     h=inline('x','t','x','u','th');
%     m=nl(f,h,[1 0 1 2])
...
if nargin<3
   error('SigSys:generic', 'NL constructor: three input arguments required')
end
if nargin<4
   fs=NaN;
end

if ~isvector(nn) | length(nn)~=4
   error('SigSys:generic', 'NL constructor: nn must be a vector of length 4')
end
nx=nn(1); nu=nn(2); ny=nn(3); nth=nn(4);
if isa(f,'inline') || isa(f, 'function_handle')
   % do nothing
elseif isstr(f)
   if exist(f,'file') || isa(f, 'function_handle')
       % do nothing
       %      f=[f,'(t,x,u,th)'];
   else
       try
          ...
          error('SigSys:generic', 'NL constructor: a string argument for f must be a valid m-file or a valid inline input argument')
       end
   end
% $$$    try
% $$$       f=inline(f,'t','x','u','th');
...
elseif isempty(f) & nx==0
   ...
   error('SigSys:generic', 'NL constructor: f must be an inline object, a string, or an m-file')
end

if isa(h,'inline') || isa(h, 'function_handle')
   % do nothing
elseif isstr(h)
   if exist(h,'file')  % m-file
      ...
      try
         ...
         error('SigSys:generic', 'NL constructor: a string argument for h must be a valid m-file or a valid inline input argument')
      end
   end
elseif isempty(h)
   h=[];
else
   error('SigSys:generic', 'NL constructor: h must be an inline object, a string, or an m-file')
end
x0=zeros(nx,1);
th=zeros(nth,1);
if nx>0
    try
       fdum=feval(f,0,1e-5*randn(size(x0)),1e-5*randn(nu,1),1e-5*randn(size(th)));
    catch ME
       disp(['NL error code: ',ME.message])
       error('SigSys:generic', 'NL constructor: sizes in nn not consistent with f')
    end
   if size(fdum,1)~=nx
      disp(['size(f): ',num2str(size(fdum))])
      disp(['size(x,1): ',num2str(nx)])
      error('SigSys:generic', 'NL constructor: size of f not consistent with nn')
   end
   if size(fdum,2)~=1
      error('SigSys:generic', 'NL constructor: f should be a column vector')
   end
   try
       fdum=feval(f,[0 1],[x0 x0],zeros(nu,2),th);
       if size(fdum,2)~=2
           error('SigSys:generic', 'NL constructor: size of f not consistent with nn')
       end
    catch ME
       warning('SigSys:generic', 'NL warning: Try to vectorize f for increased speed')
       warning('SigSys:generic', ['Hint: ',ME.message])
   end
end
   
if ny>0
   try
      hdum=feval(h,0,1e-5*randn(size(x0)),1e-5*randn(nu,1),1e-5*randn(size(th)));
    catch ME
       disp(['NL error code: ',ME.message])
      error('SigSys:generic', 'NL constructor: sizes in nn not consistent with h or error in evaluating h')
   end
   if size(hdum,1)~=ny
      disp(['size(h): ',num2str(size(hdum))])
      disp(['size(y,1): ',num2str(ny)])
      error('SigSys:generic', 'NL constructor: size of h not consistent with nn')
   end
   try
      hdum=feval(h,[0 1],[x0 x0],zeros(nu,2),th);
      if size(hdum,2)~=2
          error('SigSys:generic', 'NL constructor: size of h not consistent with nn')
      end
   catch
      warning('SigSys:generic', 'NL constructor warning: try to vectorize h for increased speed')
   end
end

if ~isnumericscalar(fs)
   error('SigSys:generic', 'NL constructor: fs must be a numeric scalar')
end
if fs<0 & ~isnan(fs)
   error('SigSys:generic', 'NL constructor: fs must be positive or NaN')
end
m.name=[];
m.desc=[];
...
function m=set.x0(m,x0);
   if ~isempty(x0) & length(x0)~=m.nn(1)
      error('SigSys:generic', 'NL: cannot change dimension of x0')
   end
   m.x0=x0(:);
end

function m=set.th(m,th);
   if ~isempty(th) & length(th)~=m.nn(4)
      error('SigSys:generic', 'NL: cannot change dimension of th')
   end
   m.th=th(:);
end

function m=set.P(m,P);
   if ~isempty(P) & length(P)~=m.nn(4)
      error('SigSys:generic', 'NL: cannot change dimension of P')
   end
   if ~iscov(P)
      error('SigSys:generic', 'NL: P is not a valid covariance')
   end
   m.P=P;
end
...
function m=set.I(m,I);
   if isempty(I)
      % nothing
   elseif  length(I)~=m.nn(1)+m.nn(4)
      error('SigSys:generic', 'NL: cannot change dimension of I (square nx+nth matrix)')
   end
   if ~iscov(I)
      warning('SigSys:generic', 'NL warning: I is not a valid information matrix (symm pos semidefinit)')
   end
   m.I=I;
end
...
function m=set.pe(m,pe);
   ...
   elseif isa(pe,'pdfclass')
       m.pe=pe;
       if length(pe)~=m.nn(3);  % length(e)=ny
          error('SigSys:generic', 'NL fieldwrite: length of symbolic pe must equal ny')
       end
   elseif iscov(pe)
       if size(pe,1)~=m.nn(3);  % length(e)=ny
          error('SigSys:generic', 'NL fieldwrite: size of covariance matrix pe must equal ny')
       end
       m.pe=ndist(zeros(m.nn(3),1),pe);
   else
      error('SigSys:generic', 'NL fieldwrite: pe must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.pv(m,pv);
   ...
   elseif isa(pv,'pdfclass')
       m.pv=pv;
       if length(pv)~=m.nn(1);  % length(e)=nx
          error('SigSys:generic', 'NL fieldwrite: length of symbolic pv must equal nx')
       end
   elseif iscov(pv)
       if size(pv,1)~=m.nn(1);
          error('SigSys:generic', 'NL fieldwrite: size of covariance matrix pv must equal nx')
       end
       m.pv=ndist(zeros(m.nn(1),1),pv);
   else
      error('SigSys:generic', 'NL fieldwrite: pv must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.px0(m,px0);
   ...
   elseif isa(px0,'pdfclass')
       m.px0=px0;
       if length(px0)~=m.nn(1);  % length(e)=nx
          error('SigSys:generic', 'NL fieldwrite: length of symbolic px0 must equal nx')
       end
   elseif iscov(px0)
       if size(px0,1)~=m.nn(1);
          error('SigSys:generic', 'NL fieldwrite: size of covariance matrix px0 must equal nx')
       end
       m.px0=ndist(zeros(m.nn(1),1),px0);
   else
      error('SigSys:generic', 'NL fieldwrite: px0 must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.fs(m,fs);
   if isnumericscalar(fs) | fs<0
      ...
      error('SigSys:generic', 'NL fieldwrite: fs must be a positive scalar or NaN')
   end
end
...
function m=set.xlabel(m,xlabel);
   ...
   elseif isempty(xlabel)
      ...
      error('SigSys:generic', ['NL fieldwrite: Field xlabel must be a cell of length nx, [], or string if nx=1'])
   end
end

function m=set.ylabel(m,ylabel);
   if ~iscell(ylabel) & isstr(ylabel) & m.nn(3)==1
      m.ylabel={ylabel};   % Cellify
   elseif iscell(ylabel) & length(ylabel)==m.nn(3)
      m.ylabel=ylabel;
   elseif isempty(ylabel)
      m.ylabel=ylabel;
   else
      error('SigSys:generic', ['NL fieldwrite: Field ylabel must be a cell of length ny, [], or string if ny=1'])
   end
end

function m=set.ulabel(m,ulabel);
   if ~iscell(ulabel) & isstr(ulabel) & m.nn(2)==1
      m.ulabel={ulabel};   % Cellify
   elseif iscell(ulabel) & length(ulabel)==m.nn(2)
      m.ulabel=ulabel;
   elseif isempty(ulabel)
      m.ulabel=ulabel;
   else
      error('SigSys:generic', ['NL fieldwrite: Field ulabel must be a cell of length nu, [], or string if nu=1'])
   end
end

function m=set.thlabel(m,thlabel);
   if ~iscell(thlabel) & isstr(thlabel) & m.nn(4)==1
      m.thlabel={thlabel};   % Cellify
   elseif iscell(thlabel) & length(thlabel)==m.nn(4)
      m.thlabel=thlabel;
   elseif isempty(thlabel)
      m.thlabel=thlabel;
   else
      error('SigSys:generic', ['NL fieldwrite: Field thlabel must be a cell of length nth, [], or string if nth=1'])
   end
end
...
function m=set.tlabel(m,tlabel);
   if  isstr(tlabel)
      ...
      error('SigSys:generic', ['NL fieldwrite: Field tlabel must be a string'])
   end
end
...
if nargin<3
   error('SigSys:generic', 'NL.ARRAYREAD: both input indeces in s(j,i) are required')
end
nu=m.nn(2);
ny=m.nn(3);
if isstr(i)
    if strcmp(i,':')
       ...
       error('SigSys:generic', 'NL.ARRARYREAD: Inputs must be a vector or :')
    end
end
if isstr(j)
    if strcmp(j,':')
       ...
       error('SigSys:generic', 'NL.ARRAYREAD: Inputs must be a vector or :')
    end
end
if any(i>nu)
     error('SigSys:generic', 'NL.ARRAYREAD: Output (row) index larger than the number of outputs')
end
if any(j>ny)
     error('SigSys:generic', 'NL.ARRAYREAD: Input (column) index larger than the number of inputs')
end
sysf=m.f;
sysh=m.h;
sysnn=m.nn;
if length(j)<ny % Delete outputs
   if ~isa(m.h,'inline')
      error('SigSys:generic', 'NL.ARRAYREAD: Removing output is not possible for m-file definition of h')
   end
   I=eye(ny);
   sysh=inline([char(h),'*eye(j,:)']);
   sysnn(3)=length(j);
end
sys=nl(sysf,sysh,sysnn);
...
function disp(m)
%DISP returns an ascii formatted version of the NL model
format='%11.2g';
if isa(m.f,'inline')
   fstr=formula(m.f);
elseif isa(m.f, 'function_handle')
   fstr = func2str(m.f);
   if fstr(1) ~= '@'
     fstr = sprintf('%s(t,x,u,th)', fstr);
   end
else
   fstr=[m.f,'(t,x,u,th)'];
end
if isa(m.h,'inline')
   hstr=formula(m.h);
elseif isa(m.h, 'function_handle')
   hstr = func2str(m.h);
   if hstr(1) ~= '@'
     hstr = sprintf('%s(t,x,u,th)', hstr);
   end
else
   hstr=[m.h,'(t,x,u,th)'];
end
...
elseif isempty(m.pe)
   ...
   pestr=[' + N(0,',mat2strformat(m.pe,format),')'];
end
if length(pestr)>20
   pestr=[' + e'];
end
...
if ~isempty(m.h) 
   ...
   ind2=find(hstr==']');
   n=length(ind);
   if n>0 & n==m.nn(3)-1 & ~strfind(hstr,'*x') % matrix with one row per measurement
...
if ~isempty(m.P) & ~all(diag(m.P(m.nn(4)+1:end,m.nn(4)+1:end))==0)
    disp(['   std = [',num2str(sqrt(diag(m.P(m.nn(4)+1:end,m.nn(4)+1:end)))',format),']'])
end
if ~isempty(m.th)
    disp(['   th''  = ',mat2strformat(m.th',format)])
end
if ~isempty(m.P) & ~all(diag(m.P(1:m.nn(4),1:m.nn(4)))==0)
    disp(['   std = [',num2str(sqrt(diag(m.P(1:m.nn(4),1:m.nn(4))))',format),']'])
end
disp('')
if ~isempty(m.xlabel)
   str='  States:  ';
...
if isnan(mc.fs)
   ...
   ind2=findstr(fc,';');
   ind=[ind1(1) ind2];
   if length(ind)~=nx, error('SigSys:generic', 'NL.C2D string interpretation problems'), end
   ind=[ind length(fc)];
   fd=fc(1:ind(1));
   for k=1:nx
   ...
   error('SigSys:generic', 'NL.C2D cannot change sampling interval of discrete time models')
end
end
...
%ADDSENSOR adds (another) sensor to a NL model 
...
%
%   Assumptions:
%      all parameters th are in s (ms.nn(4)=s.nn(4)-m.nn(4), ms.th=s.th)
...
%   ------------------------------------------
%
%   Example: radar as toa+doa
%      s=exsensor('radar');   % Direct way of radar sensor 
%      s1=exsensor('toa');
%      s2=exsensor('doa');
%      s12=addsensor(s1,s2)   % s12 is the same as s
%   Example: motion model plus radar sensor
...
nn=m.nn+[0 0 s.nn(3) s.nn(4)-m.nn(4)];
nn=m.nn;
nn(3)=m.nn(3)+s.nn(3);  % ny is the sum of outputs
nn(4)=s.nn(4);  % Default the sum, but be careful
if isa(s, 'lss')
    s = nl(s);
end
if isa(m.h,'inline') & isa(s.h,'inline')
   ...
   ms.th=s.th;
elseif isempty(m.h)
   if s.nn(4)==0, nn(4)=m.nn(4); end  % Fix for this special case
   ms=nl(m.f,s.h,nn);
   if m.nn(4)>0 & s.nn(4)>0
     warning('SigSys:generic', 'Warning: both motion and sensor models contain parameters, check the result')
      ms.th={m.th{:},s.th{:}};
   elseif m.nn(4)>0 & s.nn(4)==0
      ms.th=m.th;
   elseif m.nn(4)==0 & s.nn(4)>0
      ms.th=s.th;
   end
   ...
   error('SigSys:generic', 'nl.addsensor: cannot mix mfiles and inline representations of h with this function')
end
ms.fs=m.fs;
ms.pv=m.pv;
ms.x0=m.x0;
...
%REMOVESENSOR removes the sensors ind in a NL model 
...
elseif isa(m.h,'inline') 
   ...
   if h1(2)==';', h1(2)=[]; end %remove initial semikolon if any

   ms=m;
   ms.nn = nn;
   ms.h = inline(h1, 't', 'x', 'u', 'th');
   R=cov(m.pe);
   R(ind,:)=[];
   R(:,ind)=[];
   ms.pe=R;
   ...
   error('SigSys:generic', 'nl.removesensor: cannot mix mfiles and inline representations of h with this function')
end
end
...
%
%   Example 2: linear model, comparing ekf on nl object with kalman on ss object
%     mss=exlti('cv2d');
%     mnl=nl(mss);
%     z=simulate(mss,10);
%     zhat1=kalman(mss,z);
...
if isnan(m.fs)
    error('SigSys:generic', 'NL.EKF: only discrete time models')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL.EKF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL.EKF: Number of inputs nu must be the same in model and signal')
end
...
if isempty(m.pv) 
   warning('SigSys:generic', 'NL.EKF warning: m.pv is not defined, using v=0')
end
if isempty(m.pe) 
   warning('SigSys:generic', 'NL.EKF warning: m.pe is not defined, using e=0')
end
if isempty(opt.P0) | isnan(opt.P0)
   %warning('SigSys:generic', 'NL.EKF warning: px0 not defined, using a default value instead')
   P0=1e3*eye(nx);
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL.EKF: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL.EKF: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nx & size(opt.Q,2)==nx
   ...
   error('SigSys:generic', 'NL.EKF: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
if isempty(Q)
   Q=0;
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL.EKF: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL.EKF: Model must have a non-empty measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL.EKF: k must be a scalar k>=0')
end
...
for k=1:N
   ...
   yphat=bsxfun(@plus, m.h(t(k),xhat,u(k,:).',m.th), mean(m.pe));
   S = C*P*C' + R;
   Pyp(k,:,:) = S;
   K=(P*C')/S;
   ImKC = Inx - K*C;
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL.EKF: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL.FIM: only discrete time models')
end
nx=m.nn(1);
nu=m.nn(2);
ny=m.nn(3);
...
if isa(z,'sig') 
    if isempty(z.x)
        error('SigSys:generic', 'NL.FIM: the input SIG object z must have a state x')
    end
    x=z.x;
    t=z.t;
    [N,nyz,nuz]=size(z);
    ...
    if nx~=nxdata
       if N==nx
          ...
          N=1;
           ...
           error('SigSys:generic', ['NL.FIM: the input state vector z must must have ',num2str(nx),' elements'])
       end
    end
end

if N>1
    error('SigSys:generic', 'NL.FIM: FIM is only evaluated for one state at the time')
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL.FIM: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL.FIM: Model must have a non-empty measurement noise covariance R')
end

xsig=sig(zeros(1,ny),t,zeros(1,nu),x);
C=nlnumgrad(m,xsig,'dhdx').';
I=C'*inv(R)*C;
end
...
if nargin<2
    error('SigSys:generic', 'NL.PD: at least two input arguments required')
end
if nargin<3
   pfa=0.01;
end
if ~isnumericscalar(pfa) | pfa<=0 | pfa>=1
    error('SigSys:generic', 'NL.PD: 0<pfa<1')
end

if isnan(m.fs)
    error('SigSys:generic', 'NL.PD: only discrete time models')
end
nx=m.nn(1);
nu=m.nn(2);
ny=m.nn(3);
...
if isa(z,'sig') 
    if isempty(z.x)
        error('SigSys:generic', 'NL.PD: the input SIG object z must have a state x')
    end
    x=z.x;
    t=z.t;
    [N,nyz,nuz]=size(z);
    ...
    if nx~=nxdata
       if N==nx
          ...
          N=1;
           ...
           error('SigSys:generic', ['NL.PD: the input state vector z must must have ',num2str(nx),' elements'])
       end
    end
end

if N>1
    error('SigSys:generic', 'NL.PD: PD is only evaluated for one state at the time')
end
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL.CRLB: only discrete time models')
end
if isempty(z.x)
    error('SigSys:generic', 'NL.CRLB: the input SIG object z must have a state x')
end
...
[N,nyz,nuz]=size(z);
%if nyz~=ny
%   error('SigSys:generic', 'NL.CRLB: Number of outputs ny must be the same in model and signal')
%end
%if nuz~=nu
%   error('SigSys:generic', 'NL.CRLB: Number of inputs nu must be the same in model and signal')
%end
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL.CRLB: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL.CRLB: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nx & size(opt.Q,2)==nx
   ...
   error('SigSys:generic', 'NL.CRLB: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL.CRLB: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL.CRLB: Model must have a non-empty measurement noise covariance R')
end
if all(diag(R)==0)
   error('SigSys:generic', 'NL.CRLB: Model must have a positive definite measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL.CRLB: k must be a scalar k>=0')
end
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL.CRLB: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
%   The function calls nltf, so consult its help text
[x,V]=nltf(varargin{:},'tup','uteval','mup','uteval');
end
...
%   The function calls nltf, so consult its help text
[x,V]=nltf(varargin{:},'tup','tt2eval','mup','tt2eval');
end
...
%   The function calls nltf, so consult its help text
[x,V]=nltf(varargin{:},'tup','tt1eval','mup','tt1eval');
end

function [x,V]=nltf(m,z,varargin)
%NLTF implements the UKF and EKF without Ricatti equations
%   [x,V]=nltf(m,z,Property1,Value1,...)
%
%   m      NL object with model 
%   z      SIG object with measurements
%   x      SIG object with state estimates
...
%
%   Note: the NL object must be a function of indexed states, so always
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL.NLTF: only discrete time models')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL.NLTF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL.NLTF: Number of inputs nu must be the same in model and signal')
end
...
if isempty(m.pv) 
   warning('SigSys:generic', 'NL.NLTF warning: m.pv is not defined, using v=0')
end
if isempty(m.pe) 
   warning('SigSys:generic', 'NL.NLTF warning: m.pe is not defined, using e=0')
end
if isempty(opt.P0)  | isnan(opt.P0)
    % Use P0 from m.px0
    %warning('SigSys:generic', 'NL.NLTF warning: px0 not defined, using a default value instead')
    P0=1e3*eye(nx);
elseif isnumericscalar(opt.P0)
    P0=opt.P0*eye(nx);
elseif iscov(opt.P0)
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL.NLTF: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL.NLTF: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nx & size(opt.Q,2)==nx
   ...
   error('SigSys:generic', 'NL.NLTF: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
if isempty(Q)
   Q=0;
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL.NLTF: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL.NLTF: Model must have a non-empty measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL.NLTF: k must be a scalar k>=0')
end
...
elseif strncmpi(opt.mup,'mc',2) 
   ...
   error('SigSys:generic', ['Unknown option for mup: ',opt.mup])
end
...
elseif strncmpi(opt.tup,'mc',2) 
   ...
   error('SigSys:generic', ['Unknown option for tup: ',opt.tup])
end
...
for k=1:N
   ...
   if isa(m.h, 'function_handle')
     fbar = @(x, t, u, th)[x(1:nx, :); m.h(t, x, u, th) + x(nx+1:end, 1)];
   else
     h=char(m.h);
   ...
   Ez=E(Z);
   Ez(nx+1:end) = bsxfun(@plus, Ez(nx+1:end), mean(m.pe));
   Pz=cov(Z);
   yfhat=y(:,k);
   yphat=Ez(nx+1:end);
   P=Pz(1:nx,1:nx);
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL.NLTF: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
function [zhat,xhist]=pf(m,z,varargin)
%PF implements the particle filter for state estimation
%   [zhat,xhist]=pf(m,z,Property1,Value1,...)
%
%   m      NL object with model 
%   z      SIG object with measurements
%   x      SIG object with state estimates
%          xhat=x.x and signal estimate yhat=x.y
%   xhist  Three-dim (Np x N x nx) matrix with particle history
%
%   Property   Value      Description
%   ------------------------------------------
...
%            'residual'
%            'stratified'
%   Nth      {1}        Relative threshold, resample if Neff/N < Nth
%   marginal {'off'},'on' Compute marginal distribution
...
%            'traj'       Plot of particle trajectories x(1:k,ind(i)) 
%
%   Example 1: non-linear model
%     m=exnl('ctpv2d'); % Coordinated turn model
%     z=simulate(m,10); % Ten seconds trajectory
...
%     xplot2(z,zhat1,zhat2,'conf',90)
%
%   Example 3: Animate depletion illustration
%     m=exnl('pfex');
%     y=simulate(m,10);
%     pf(m,y,'animate','on','delay',0.5,'type','traj');
%     xplot(y,'linewidth',4);  % Compare with true trajectory
%
%   Example 4: Visualize depletion by particle history at the end
%     ml=lss(0.9,0,1,0,0.01,0.0001,1)
%     mn=nl(ml)
%     y=simulate(ml,10);
%     [xpf1,xhist1]=pf(mn,y,'Np',100,'proposal','sir');
%     [xpf2,xhist2]=pf(mn,y,'Np',100,'proposal','opt');
%     subplot(2,1,1)
%     plot(1:N,squeeze(xhist1)','b',1:N,y.x,'k')
%     subplot(2,1,2)
%     plot(1:N,squeeze(xhist2)','b',1:N,y.x,'k')

opt=struct('k',0,'Np',100,'sampling','simple','proposal','sir','Nth',1,'marginal','off','animate','off','ind',[],'delay',0,'type','xplot','Xlim',[],'Ylim',[]);
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL.PF: only discrete time models')
end

if ~isnumericscalar(opt.k) | opt.k<0 | opt.k>1
   error('SigSys:generic', 'NL.PF: k must be 0 or 1')
end
if ~isnumericscalar(opt.Np) | opt.Np<1 
   error('SigSys:generic', 'NL.PF: Np must a positive scalar')
end

if isempty(opt.ind)
   ind=':';
elseif ~isnumeric(opt.ind)
   error('SigSys:generic', 'NL.PF: value for argument animate must be numeric')
else
   ind=opt.ind;
end
if ~isnumeric(opt.delay) | opt.delay<0
   error('SigSys:generic', 'NL.PF: delay must be numeric and positive')
end
...
Np=opt.Np;
if nyz~=ny
   error('SigSys:generic', 'NL.PF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL.PF: Number of inputs nu must be the same in model and signal')
end
...
for k=1:N;
   ...
   if strcmp(opt.marginal,'on') & k>1
       for i=1:Np
           for j=1:Np
               wij(i,j)=pdf(m.pv,squeeze(Px(i,k,:)-Px(j,k-1,:)));  % Prior
           end
       end
       w=wij*w;
   end
   ...
   if strcmp(opt.animate,'on')
      ...
      elseif strcmp(opt.type,'xplot2')
         ...
         if length(ind)~=2
            error('SigSys:generic', 'NL.PF: ind must have length 2 in xplot2')
         end
      ...
      elseif strcmp(opt.type,'plot')
         ...
         drawnow
         pause(opt.delay)
      elseif strcmp(opt.type,'traj')
         if strcmp(ind,':'), ind=1:nx; end
         for i=1:length(ind)
            subplot(length(ind),1,i)
            if k>1
                plot([t(k-1) t(k)],[xpold(:,ind(i)) xp(:,ind(i))]')
            end
            if strcmp(opt.marginal,'on') & k>1
                for ii=1:Np, for jj=1:Np
                     plot([t(k-1) t(k)],[xpold(ii,ind(i)) xp(jj,ind(i))]','--')
                end, end
             end
            title([z.xlabel{ind(i)}])
            hold on
         end
         drawnow
         pause(opt.delay)
   ...
   w=w/sum(w);
   if any(isnan(w))
     warning('nl:pf', 'All weights are zero, keeping current particles');
     w(:) = 1/numel(w);
   end
   Neff=1/sum(w.^2);
   if Neff/opt.Np < opt.Nth
       [xp,w,indresampling]=resampling(xp,w,opt.sampling);  % Resampling
       xhist=xhist(indresampling,:,:);
   end
   xpold=xp;
   if opt.k==0
      % Compute y(k|k)
        ...
        if size(yp, 1) ~= Np % For some implementations, vectorized call works but is not vectorized..
           for i=1:Np
              yp(i,:)=feval(m.h,t(k),xp(i,:).',u(:,k),m.th).';
           end 
        end
   ...
   elseif strncmpi(opt.proposal,'opt',3)
      ...
      if k<N;
         ...
         try  % vectorized call
            ...
            for i=1:Np
               yp(i,:)=feval(m.h,t(k+1),xp(i,:).',u(:,k+1),m.th).';
            end
         end
         epsi=repmat(y(:,k+1).',Np,1)-yp;     % prediction error
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL.PMF: only discrete time models')
end

if ~isnumericscalar(opt.k) | opt.k<0 | opt.k>1
   error('SigSys:generic', 'NL.PMF: k must be 0 or 1')
end
if ~isnumericscalar(opt.Np) | opt.Np<1 
   error('SigSys:generic', 'NL.PMF: Np must a positive scalar')
end

if isempty(opt.ind)
   ind=':';
elseif ~isnumeric(opt.ind)
   error('SigSys:generic', 'NL.PMF: value for argument animate must be numeric')
else
   ind=opt.ind;
end
if ~isnumeric(opt.delay) | opt.delay<0
   error('SigSys:generic', 'NL.PMF: delay must be numeric and positive')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL.PMF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL.PMF: Number of inputs nu must be the same in model and signal')
end
Np=opt.Np;
if nx==1
...
elseif nx==2
  ...
  pv=reshape(pv,N1,N1);
   ...
   error('SigSys:generic', 'NL.PMF: nx must be less than or equal to 2 in the PMF')
end
...
%CALIBRATE computes the NLS parameter estimate in the nl (or sensormod) object s from y
...
%
%   See help nls for options on property-value pairs
%
%   Example:
...
%   Calls nl.estimate with x0mask=zeros(nx,1)
nx=s.nn(1);
...
%   The NLS algorithm is used, which applies the Gauss-Newton algorithm for the
%   parameters and initial states
%
%   m  is an NL object
%   z   is a SIG object
%   res is the output struct from the NLS function
%
...
%
%   The optional values are passed to nls. See help nls for other options.
%
%   See also:
...
if nargin<2
   error('SigSys:generic', 'NL estimate: two input arguments required')
elseif ~isa(m,'nl')
   error('SigSys:generic', 'NL estimate: input argument 1 must be an NL object')
elseif ~isa(z,'sig')
   if iscell(z)
      for kk=1:length(z)
         if ~isa(z{kk},'sig')
            error('SigSys:generic', 'NL estimate: input argument 2 must be (an array of) SIG object')
         end
      end
   else
      error('SigSys:generic', 'NL estimate: input argument 2 must be a SIG object')
   end
end
[mhat,res]=nls(m,z,varargin{:});
if ~all(all(m.I==0))
    % Fusion of information in m and mhat
    % Uses that NLS is ML, not Bayesian, so P from m is not used
    I0=m.I;
    th0=m.th;
    x0=m.x0;
    I1=mhat.I;
...
%SIMULATE simulates the NL system
...
%   If the NL system contains no input, use T=[t0 tfinal] instead of u.
...
if nargin<2
   error('SigSys:generic', 'NL.simulate: two input arguments required')
end
opt=struct('MC',30);
opt=optset(opt,varargin);
...
if isa(in1,'sig')
   ...
   if ~isnan(in1.fs) & in1.fs~=m.fs
      error('SigSys:generic', 'NL.simulate: m and z must have the same sampling frequency')
   end
   if size(u,1) ~= nu
      error('SigSys:generic', 'NL.simulate: input dimension of m must match output dimension of z')
   end
elseif isvector(in1)
   ...
   elseif length(in1)==2
      ...
      if any(abs(diff(t)-Ts)>1e-12)
         error('SigSys:generic', 'NL.simulate: second input as a time vector must have fs consistent with the model')
      end
      if m.fs~=1/Ts;
          error('SigSys:generic', 'The sample sampling frequency of m does not correspond to the one in the time vector')
      end
   end
   if nu>0
      u=zeros(nu,length(t));
   ...
   error('SigSys:generic', 'NL.simulate: second input argument must be a vector or SIG object')
end
N=length(t);
t=t(:);
...
if isnan(m.fs)  % Continuous time system
   ...
   if nx>0
      if nu>0
         [T,X]=ode45(@(varargin)m.f(varargin{:}),[t(1) t(2)],m.x0,[],[],u(:,1),m.th);
         x=[m.x0 X(end,:).'+v(:,1)];
         for k=2:N-1
            [Tnew,Xnew]=ode45(@(varargin)m.f(varargin{:}),[t(k) t(k+1)],x(:,end),[],[],u(:,k),m.th);
            T=[T; Tnew(:)];
            X=[X; Xnew];
            x=[x Xnew(end,:).'+v(:,k)];
         end
      else
	[T,X]=ode45(@(varargin)m.f(varargin{:}),[t(1) t(2)],m.x0,[],[],m.th);
         x=[m.x0 X(end,:).'+v(:,1)];
         for k=2:N-1
            [Tnew,Xnew]=ode45(@(varargin)m.f(varargin{:}),[t(k) t(k+1)],x(:,end),[],[],m.th);
            T=[T; Tnew(:)];
            X=[X; Xnew];
            x=[x Xnew(end,:).'+v(:,k)];
         end
   ...
   if nx>0
      ...
      if nu>0
         for k=1:N-1
            x(:,k+1)=feval(m.f,t(k),x(:,k),u(:,k),m.th)+v(:,k);
         end
      else
         for k=1:N-1
            x(:,k+1)=feval(m.f,t(k),x(:,k),[],m.th)+v(:,k);
         end
      end
   else
      x=[];
...
if nx>0
   try  % Vectorized definition, which runs faster
      y=feval(m.h,t(:),x,u,m.th);
   end
   if size(y,2)~=ny | size(ny,1)~=length(t) % loop over time
      if nu>0
         for k=1:N
             y(:,k)=feval(m.h,t(k),x(:,k),u(:,k),m.th);
         end
      else
         for k=1:N
             y(:,k)=feval(m.h,t(k),x(:,k),[],m.th);
         end
      end
   end
else % static system
      if nu>0
         try  % Vectorized definition, which runs faster
            y=feval(m.h,t(:),[],u,m.th);
         catch
            for k=1:length(t);
               y(:,k)=feval(m.h,t(k),[],u(:,k),m.th);
            end
         end
      else
         try  
      ...
      elseif size(y,2)==ny & size(y,1)==length(t) 
         ...
         error('SigSys:generic', 'NL.SIMULATE: incorrect output from h')
      end
end
if isa(m.pe,'pdfclass')
...
function [mout,zout]=nl2lss(m,z)
%NL2SS returns a linearized state space model
%   [mout,zout]=nl2lss(m,z)
%   
...
%
%   Example:
%     mnl=nl('-x(1,:).^2-x(1,:)','x',[1 0 1 0])
%     mss=nl2lss(mnl,sig(0,mnl.fs,[],2))
%
%   See also: lss.lss2nl

if nargin<2
   error('SigSys:generic', 'NL nl2lss: two input arguments required')
elseif ~isa(m,'nl')
   error('SigSys:generic', 'NL nl2lss: first input argument must be an NL object')
elseif ~isa(z,'sig')
   error('SigSys:generic', 'NL nl2lss: second input argument must be a SIG object')
elseif size(z,1)>1
   error('SigSys:generic', 'NL nl2lss: SIG object z has to be one sample, not a signal')
elseif  isempty(z.x)
   error('SigSys:generic', 'NL nl2lss: SIG object z must contain a state z.x')
end
...
if size(z.x,2)~=nx
   error('SigSys:generic', 'NL nl2lss: SIG object state z.x does not match the state dimension in m')
elseif nu>0 & isempty(z.u)
   error('SigSys:generic', 'NL nl2lss: model m has an input, but the SIG object z.u is empty')
elseif nu~=size(z.u)
   error('SigSys:generic', 'NL nl2lss: model m input has not the same dimension as the SIG object z.u')
end


A=nlnumgrad(m,z,'dfdx')';
C=nlnumgrad(m,z,'dhdx')';
if nu>0
   B=nlnumgrad(m,z,'dfdu')';
   D=nlnumgrad(m,z,'dhdu')';
...
function der=nlnumgrad(m,z,derstr,mask)
%NLNUMGRAD returns a numeric gradient approximation
%   Syntax                     Approx  Dim         Application
%   dfdx=nlnumgrad(m,z,'dfdx')   df/dx  (nx,nx,N)    A(t)' in EKF
%   dfdu=nlnumgrad(m,z,'dfdu')   df/du  (nu,nx,N)    B(t)' in EKF
%   dhdx=nlnumgrad(m,z,'dhdx')   dh/dx  (nx,ny,N)    C(t)' in EKF
%   dhdu=nlnumgrad(m,z,'dhdu')   dh/du  (nu,ny,N)    D(t)' in EKF
%   dfdth=nlnumgrad(m,z,'dfdth') df/dth (nth,nx,N)   Estimation
%   dhdth=nlnumgrad(m,z,'dhdth') dh/dth (nth,ny,N)   Estimation
%   dxdth=nlnumgrad(m,z,'dxdth') dx/dth (nth,nx,N)   Estimation
%   dydth=nlnumgrad(m,z,'dydth') dy/dth (nth,ny,N)   Estimation
%   dxdx0=nlnumgrad(m,z,'dxdx0') dx/dx0 (nx,nx,N)    Estimation
%   dydx0=nlnumgrad(m,z,'dydx0') dy/dx0 (nx,ny,N)    Estimation
...
if nargin<3
   error('SigSys:generic', 'NL nlnumgrad: at least three input arguments required')
elseif ~isa(m,'nl')
   error('SigSys:generic', 'NL nlnumgrad: first input argument must be an NL object')
elseif ~isa(z,'sig')
   error('SigSys:generic', 'NL nlnumgrad: second input argument must be a SIG object')
elseif ~isstr(derstr)
   error('SigSys:generic', 'NL nlnumgrad: third input argument must be a string')
end
...
if strcmpi(derstr,'dfdx') | strcmpi(derstr,'dhdx')
    ...
    if sum(mask)~=length(find(mask)), 
       error('SigSys:generic', 'NL nlnumgrad: mask must be binary'),
    end
    n=sum(mask); 
...
elseif strcmpi(derstr,'dxdth') | strcmpi(derstr,'dydth')
    ...
    for j=1:n
     ...
     error('SigSys:generic', 'NL nlnumgrad: unknown option for der')
end
end
