...
%#
      ...
      elseif nargin==2
        ...
        if ~isscalar(s)
          error('SigSys:generic', 'LOGDIST: s must be scalar')
        end
        if  ~isreal(s)
          error('SigSys:generic', 'LOGDIST: s must be a real')
        end
        if s<=0
          warning('SigSys:generic', 'LOGDIST warning: s must be positive')
        end
        X.s = s;   %%%
        X.mu = mu; %%%
      else
        error('SigSys:generic', 'Syntax: logdist(mu,s), or just logdist')
      end
    ...
    function X=set.s(X,s)
      if ~isempty(X.s)
          if length(s)~=length(X.s)
             error('SigSys:generic', ['Cannot change the dimensions of s'])
          elseif ~isscalar(s) | s<=0
              error('SigSys:generic', ['s is not a valid scaling'])
          end
      end
      X.s=s;
    end
...
%   i is the row index/indices
    ...
    function x=rand(X,n,m)
      ...
      if isempty(n) | ~isreal(n) | n<0 | round(n)~=n
        error('SigSys:generic', 'logdist.rand: n must be a non-negative integer'),
      end
      if isempty(X.mu) | isempty(X.s)
          error('SigSys:generic', ['logdist.rand: distribution parameters not ' ...
                 'specified'])
      end
    ...
    function I=cdf(X,x)
      ...
      if nargin<2
        error('SigSys:generic', 'Syntax: cdf(X,x)')
      end
    ...
    function x=cdfinv(X,I)
      ...
      %   That is, I(x)=P(X<x)
      if nargin<2
        error('SigSys:generic', 'Syntax: cdfinv(X,I)')
      end
      if length(X)>1
    ...
    function [p,x]=pdf(X,x)
      ...
      if nargin<2 | isempty(x)
          ...
          if (size(x,2)~=length(m)) & length(m)~=1
            error('SigSys:generic', 'LOGDIST.PDF: x must have the same number of columns as length(mu)')
          end
      end
      e=exp(-(x-m)./s);
      p=e./s./(1+e).^2;
    ...
    function Y=vertcat(X1,varargin)
      ...
      if nargin==1
        Y=X1; %%% copy or not??? /ps
        %error('SigSys:generic', 'LOGDIST.VERTCAT: at least two input arguments required')
      else
        X2=varargin{1};
        if isa(X2,'logdist')
    ...
    function Y=plus(X1,X2)
	  ...
	  Y=X2+X1;   % Use the code in gmdist
        ...
        error('SigSys:generic', 'Incorrect argument to PLUS in LOGDIST class')
      end
    end
