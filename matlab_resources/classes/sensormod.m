...
if isa(h,'sensormod')
   ...
   m.name=h.name;
elseif nargin==1
   error('SigSys:generic', 'sensormod: two input arguments required')
else
   mnl=nl('x',h,nn,NaN);
...
if nargin<2
   try
      ...
      error('SigSys:generic', 'SENSORMOD.simulate: time argument required, s(0) not defined')
   end
end
opt=struct('MC',30);
...
if nargin<2, error('SigSys:generic', 'SENSORMOD.DETECT: a third data argument y required'), end
...
[nx,nroc]=size(x0);
if nx~=s.nn(1)
   error('SigSys:generic', 'SENSORMOD.ROC: x0 must be a nx times nroc matrix')
end
Z=chi2dist(s.nn(1));
if nargin<3
   N=100;
...
if pfa<=0 | pfa >=1
   error('SigSys:generic', 'sensormod.pdplot1: pfa must be in (0,1)')
end
if nargin<2
   error('SigSys:generic', 'sensormod.pdplot1: Two input arguments needed at least, pdplot1(s,x1)')
end
...
if pfa<=0 | pfa >=1
   error('SigSys:generic', 'sensormod.pdplot2: pfa must be in (0,1)')
end
if nargin<3
   error('SigSys:generic', 'sensormod.pdplot2: Three input arguments needed at least, pdplot2(s,x1,x2)')
end
...
if isempty(s.pe)
   error('SigSys:generic', 'sensormod.ls: pe must be defined')
end
for k=1:length(y.t)
...
if isempty(s.pe)
   error('SigSys:generic', 'sensormod.wls: pe must be defined')
end
R=cov(s.pe);
Rinv=pinv(R);
for k=1:length(y.t)
...
if nargin<2 | isempty(y)
  ...
  if length(y)>1
     error('SigSys:generic', 'CRLB can only be computed for one sample at the time')
  end
  x0=y.x;
  u=y.u;
end
...
if length(ind)~=2
   error('SigSys:generic', 'SENSORMOD.CRLB2: ind must be a vector of length 2')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SENSORMOD.CRLB2: ind must be a vector with positive integers')
end
if  any(ind>s.nn(1))
   error('SigSys:generic', 'SENSORMOD.CRLB2: The state indeces in ind cannot exceed nx=s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SENSORMOD.CRLB2: pe cannot be empty for crlb2')
end
if ~isstr(type)
   error('SigSys:generic', 'SENSORMOD.CRLB2: type must be a string')
end
...
if isempty(y)
  ...
  if length(y)>1
     error('SigSys:generic', 'SENSORMOD.CRLB2 can only be computed for one sample at the time')
  end
  x0=y.x';
  u=y.u';
end
...
for i=1:N;
   ...
   case 'max'
      cx(i)=max(eig(P));
   otherwise
       error('SigSys:generic', ['SENSORMOD.CRLB2: unknown option type = ',type])
   end
end
cx=reshape(cx,N2,N1);
if nargout==0
    cxsort=sort(cx(:));
    lev=cxsort(10:fix(N/10):N);
...
if length(ind)>1
   error('SigSys:generic', 'SENSORMOD.LH1: ind must be an integer')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SENSORMOD.LH1: ind must be a positive integer')
end
if ind>s.nn(1)
   error('SigSys:generic', 'SENSORMOD.LH1: state index ind cannot exceed nx in s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SENSORMOD.LH1: pe cannot be empty for lh1')
end
if s.nn(1)~=y.nn(1)
   error('SigSys:generic', ['SENSORMOD.LH1: model and data do not have the same ' ...
...
if s.nn(3)~=y.nn(3)
   error('SigSys:generic', ['SENSORMOD.LH2: model and data do not have the same ' ...
          'measurement dimension ny'])
end
if length(ind)~=2
   error('SigSys:generic', 'SENSORMOD.LH2: ind must be a vector of length 2')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SENSORMOD.LH2: ind must be a vector with positive integers')
end
if any(ind)>s.nn(1)
   error('SigSys:generic', 'SENSORMOD.LH2: state index ind cannot exceed nx in s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SENSORMOD.LH2: pe cannot be empty for lh2')
end
[X1, X2]=meshgrid(x1, x2);
sz = size(X1);
N = numel(X1);
...
for n=1:N;
   ...
   if ~( isa(s,'sensormod') | isa(s,'nl'))
      error('SigSys:generic', 'SENSORMOD.PLOT: input argument must be a NL/SENSORMOD object')
   end
