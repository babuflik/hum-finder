...
if isa(h,'sigmod')
   ...
   m.name=h.name;
elseif nargin==1
   error('SigSys:generic', 'sigmod: two input arguments required')
else
...
if nargin<2
   try
      ...
      error('SigSys:generic', 'SIGMOD.simulate: time argument required, s(0) not defined')
   end
end
opt=struct('MC',30);
...
if length(ind)~=2
   error('SigSys:generic', 'SIGMOD.CRLB2: ind must be a vector of length 2')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SIGMOD.CRLB2: ind must be a vector with positive integers')
end
if  any(ind>s.nn(1))
   error('SigSys:generic', 'SIGMOD.CRLB2: The state indeces in ind cannot exceed nx=s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SIGMOD.CRLB2: pe cannot be empty for crlb2')
end
if ~isstr(type)
   error('SigSys:generic', 'SIGMOD.CRLB2: type must be a string')
end
...
if isempty(y)
  ...
  if length(y)>1
     error('SigSys:generic', 'SIGMOD.CRLB2 can only be computed for one sample at the time')
  end
  x0=y.x';
  u=y.u';
end
...
for i=1:N;
   ...
   case 'max'
      cx(i)=max(eig(P));
   otherwise
       error('SigSys:generic', ['SIGMOD.CRLB2: unknown option type = ',type])
   end
end
cx=reshape(cx,N1,N2);
if nargout==0
...
if length(ind)>1
   error('SigSys:generic', 'SIGMOD.LH1: ind must be an integer')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SIGMOD.LH1: ind must be a positive integer')
end
if ind>s.nn(1)
   error('SigSys:generic', 'SIGMOD.LH1: state index ind cannot exceed nx in s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SIGMOD.LH1: pe cannot be empty for lh1')
end
N=length(x1);
X=repmat(s.x0,1,N);
X(ind,:)=x1(:)';
...
if length(ind)~=2
   error('SigSys:generic', 'SIGMOD.LH2: ind must be a vector of length 2')
end
if ~isequal(ind,round(ind)) | any(ind<=0)
   error('SigSys:generic', 'SIGMOD.LH2: ind must be a vector with positive integers')
end
if any(ind)>s.nn(1)
   error('SigSys:generic', 'SIGMOD.LH1: state index ind cannot exceed nx in s.nn(1)')
end
if isempty(s.pe)
   error('SigSys:generic', 'SIGMOD.LH2: pe cannot be empty for lh2')
end
[X1,X2]=meshgrid(x1,x2);
X11=X1(:);
X22=X2(:);
...
for n=1:N;
   ...
   if ~( isa(s,'sigmod') | isa(s,'nl'))
      error('SigSys:generic', 'SIGMOD.PLOT: input argument must be a NL/SIGMOD object')
   end
