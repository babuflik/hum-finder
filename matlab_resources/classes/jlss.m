classdef jlss %< linmod
%JLSS Jump Linear State Space model
%   m=jlss(m{:}) or m=jlss(m1,m2,...)
%
%   Input arguments m{i} are linear systems of the same dimensions
%   The model is given by
%      y[k]  = m{i}.C x[k]+m{i}.D u[k]+e[k],  e[k]~N(0,m{i}.R)
%     x[k+1] = m{i}.A x[k]+m{i}.B u[k]+v[k],  v[k]~N(0,m{i}.Q)
%   Here i~[1,2,..,length(m)] is a discrete Markov sequence
%   with transition matrix Pi (default Pi(i,j)=1/length(m))
%
%   Each filter recursion starts with a measurement update and ends with
%   a time update, hence the order of the equations above.
%
%   Example:
%   m{1}=exlti('cv2d');  % Manuevre mode
%   m{2}=m{1};
%   m{2}.Q=m{2}.Q*0;     % Linear mode
%   jm=jlss(m{:});
%
%
...
function m=jlss(varargin)
%JLSS Construct a Jump Linear State Space object

%#
%#

if nargin<1
%    error('SigSys:generic', 'JLSS contructor expects an input argument: see help jlss')
else
   M=nargin;
   for i=1:M
       try
	   m{i}=varargin{1};
           ...
           warning('SigSys:generic', ['LSS error code: ',ME.message])
           error('SigSys:generic', ['JLSS constructor: Input argument ',num2str(i),' is not a valid LSS object'])
       end
   end
   for i=2:M
      if ~isequal(m{1}.nn,m{i}.nn)
	   error('SigSys:generic', ['JLSS constructor: model ',num2str(i),' does not have the same nn as model 1'])
      end
   end
   Pi=ones(M)/M;
end
...
switch arg1
    case {'A', 'B', 'C', 'D', 'Q', 'R', 'S', 'nn'}
        if ~isnumeric(arg2)
            error('SigSys:generic', 'Field value must be numeric.')
        end
end
if strcmp(arg1,'sysMC')
      ...
      elseif ~isa(arg2,'cell')
         error('SigSys:generic', 'sysMC must be a cell array')
      elseif ~isa(arg2{1},'lss')
         error('SigSys:generic', 'sysMC must be a cell array of LSS objects')
      elseif isequal(arg2{1}.nn,nn)  % Check sysMC argument
         sysMC=arg2;
         MC=length(sysMC);
      else
         error('SigSys:generic', 'Argument of sysMC does not match the LSS dimensions')
      end
elseif ~all(size(arg2)==size(eval(arg1)))
   error('SigSys:generic', 'Cannot change size of protected fields in LSS objects')
else
...
if nargin<1;
    error('SigSys:generic', 'No system input argument')
elseif nargin<2;
    sys=varargin{1};
    ...
    if isa(s1,'lss') | isa(s2,'lss')
        ...
        if ny1~=ny2
            error('SigSys:generic', ['Incorrect dimensions in horzcat. ny1=ny2 required, but ny1=',...
...
elseif nargin<2;
    ...
    if isa(s1,'lss') | isa(s2,'lss')
        ...
        [nx2,nu2,nv2,ny2]=size(s2);
        if nu1~=nu2
            error('SigSys:generic', ['Incorrect dimensions in horzcat. nu1=nu2 required, but nu1=',...
...
%   calls s^n
if nargin<2; error('SigSys:generic', 'LSS.POWER needs two input arguments'), end
sys=s^n;
...
if nargin<2; error('SigSys:generic', 'LSS.MPOWER needs two input arguments'), end
if round(n)~=n
    error('SigSys:generic', 'Power must be an integer')
end
if ~isreal(n)
    error('SigSys:generic', 'Power must be a real number')
end
if n==0;
    sys=ltf('unit');
elseif n<0
...
if isa(s1,'lss') & isa(s2,'lss')
    ...
    error('SigSys:generic', 'Both s1 and s2 must be LSS objects for EQ')
end
end
...
elseif isnumericscalar(s2)
    ...
    [nx2,nu2,nv2,ny2]=size(s2);
    if  ny1~=ny2
        error('SigSys:generic', 'Systems s1 and s2 are not compatible for parallel connection (+): ny1~=ny2')
    end
    if  nu1~=nu2
        error('SigSys:generic', 'Systems s1 and s2 are not compatible for parallel connection (+): nu1~=nu2')
    end
    a = [s1.A zeros(nx1,nx2); zeros(nx2,nx1) s2.A];
...
elseif isnumeric(s2)
    ...
    fs=s1.fs;
    if  ny2~=nu1
        error('SigSys:generic', ['In series connection (*), ny2~=nu1, but ny2=',num2str(ny2),' nu1=',num2str(nu1)])
    end
...
[nx,nu,nv,ny]=size(s);
if nv>0
    error('SigSys:generic', 'No inverse for stochastic systems')
end
...
if ~isequal(size(T),[nx nx])
    error('SigSys:generic', 'LSS.transform: T must be square with size nx')
end
if rank(T)<nx
    error('SigSys:generic', 'LSS.transform: T must have full rank')
end
Tinv=inv(T);
if nv==0
...
if  ny1~=nu2
    error('SigSys:generic', 'Systems s1 and s2 are not compatible for feedback connection: ny1~=nu2')
end
if  ny2~=nu1
    error('SigSys:generic', 'Systems s1 and s2 are not compatible for feedback connection: ny2~=nu1')
end
...
if s.fs>0
    error('SigSys:generic', 'LSS object already in discrete time')
end
...
if isnan(s.fs)
    error('SigSys:generic', 'LSS object already in continuous time')
end

[nx,nu,nv,ny]=size(s);
Ad=s.A; Bd=s.B; Cd=s.C; Dd=s.D; fs=s.fs;
if isnan(fs) | ~isnumericscalar(fs)
    error('SigSys:generic', 'fs must be specified as a scalar')
end
T=1/fs;
method=lower(method);
if strcmp(method(1:3),'zoh');
    ...
    D=Dd;
elseif strcmp(method(1:3),'foh');
    error('SigSys:generic', 'd2c with FOH is not available')
elseif strcmp(method(1:3),'bil');
...
elseif strcmp(method(1:3),'ssb');
    ...
    error('SigSys:generic', 'Unknown method')
end
...
A=s.A; B=s.B; C=s.C; D=s.D; Q1=s.Q; R1=s.R; S1=s.S; fs=s.fs; nn=s.nn;
if nn(2)==0
   error('SigSys:generic', 'lss.balreal requires a system with at least one input')
end
P=gram(s,'c');
Q=gram(s,'o');
[Up,Sp,Vp]=svd(P);
...
elseif method~=1 & method~=2
   error('SigSys:generic', 'LSS.GRAM: method must be 0,1 or 2')
end
...
elseif strncmpi(opt,'c',1)
    ...
    error('SigSys:generic', 'Unknown option, ''c'' or ''o'' are allowed as second argument')
end
if s.fs>0 %discrete time
    ...
    % Stationary P
        ...
        if n==maxiter; warning('SigSys:generic', 'Warning: maximum number of iterations attained in GRAM'), end
    else
        b=B*B';
...
if any(any(isnan(P))) | any(any(isinf(P)))
   error('SigSys:generic', 'LSS.GRAM: No solution found, system is probably unstable')
end
end
...
elseif opt.method~=1 & opt.method~=2
   error('SigSys:generic', 'LSS.LQE: method must be 0,1, 2 or 3')
end
...
fs=s.fs;
if any(any(S~=0))
    error('SigSys:generic', 'LSS.LQE is not working for S~=0')
end
if all(all(Q==0)) & all(all(R==0))
    error('SigSys:generic', 'LSS.LQE works only for stochastic systems (Q~=0, R~=0)')
end

if fs>0
    error('SigSys:generic', 'LSS.LQE does not apply for discrete time systems, use DLQE instead')
end
if rank(R)<ny
    error('SigSys:generic', 'R must have full rank')
end
...
elseif opt.method==2
   ...
   while norm(e)>1e-10*norm(Pn) & iter<maxiter
      ...
      if any(any(isnan(e))) | any(any(isinf(e)))
          error('SigSys:generic', 'LSS.LQE: Inf or NaN obtained in P iterations')
      end
   ...
   K=P*C'*inv(R);
   if  norm(e)>1e-8
      warning('SigSys:generic', ['LSS.LQE warning: remaining error ||AP+PA''-PC''R^(-1)CP+Q||=',num2str(norm(e))])
   end
   if iter==maxiter
      %error('SigSys:generic', 'LSS.LQE: maximum number of iterations used')
   end
elseif opt.method==3
   ...
   error('SigSys:generic', 'LSS.LQE: method must be 1,2 or 3')
end
if any(any(isnan(P))) | any(any(isinf(P)))
   error('SigSys:generic', 'System must be stable to compute Gramians')
end
end
...
nx=s.nn(1); fs=s.fs;
if any(any(S~=0))
    error('SigSys:generic', 'LSS.DLQE is not working when S~=0')
end
if all(all(Q==0)) & all(all(R==0))
    error('SigSys:generic', 'LSS.DLQE works only for stochastic systems (Q~=0, R~=0)')
end
Pp=eye(nx);
Ppold=zeros(nx);
n=0;
maxiter=1000;
if isnan(fs)
    error('SigSys:generic', 'LSS.DLQE does not apply for continuous time systems')
end
...
if n==maxiter;
    warning('SigSys:generic', 'Warning: maximum number of iterations attained in DLQE'),
end
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'lss.kalman: only discrete time models')
end
[nx,nu,nv,ny]=size(m);
[A,B,C,D,Q,R,S,fs]=getss(m);
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'LSS.KALMAN: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'LSS.KALMAN: Number of inputss nu must be the same in model and signal')
end
...
elseif isnumeric(opt.P0) & size(opt.P0,1)==nx & size(opt.P0,2)==nx & norm(opt.P0-opt.P0')<1e-10
   ...
   error('SigSys:generic', 'LSS.KALMAN: P0 must be empty, a scalar, or a symmetric (nx,nx) matrix')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'LSS.KALMAN: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nx & size(opt.Q,2)==nx
   ...
   error('SigSys:generic', 'LSS.KALMAN: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'LSS.KALMAN: R must be empty, a scalar, or a covariance (ny,ny) matrix')
end

if isnumericscalar(opt.k) & opt.k>=0
   k=opt.k;
else
   error('SigSys:generic', 'LSS.KALMAN: k must be a scalar k>=0')
end
if opt.alg<1 | opt.alg>5 | opt.alg~=round(opt.alg)
   error('SigSys:generic', 'LSS.KALMAN: alg must be an integer in [1,5]')
end
...
if opt.alg==1;  % Stationary KF
  ...
  [yhat,xhat]=ssdlsim(A-A*K*C,[K B-A*K*D],C,[zeros(ny,ny) D],[y u;zeros(1,ny+nu)],xhat);
...
if opt.alg==2
  ...
  for t=1:N,
    ...
    xp(:,t+1)=xhat; % xhat(t+1|t)
    Ppx(t+1,:,:)=0.5*(P+P.');
    yp(:,t+1)=C*xhat+D*u(t,:).';   % yhat(t+1|t)
    Pyt=C*P*C'+R;
    Ppy(t+1,:,:)=0.5*(Pyt+Pyt.');
...
if opt.alg==3
  if k>1,
       error('SigSys:generic', 'lss.kalman: square root algorithm must have k=0 or k=1'),
  end
  yhat=zeros(ny,N);
  Epsi=zeros(ny,N);
  Epsin=zeros(ny,N);
...
if opt.alg==4  % Smoothing
  if k~=0,
       warning('SigSys:generic', 'Warning: Kalman smoothing must have prediction horizon k=0'),
  end
  % Forward recursion
  xhatf=kalman(m,z);
  xhatp=kalman(m,z,'k',1);
  xf=xhatf.x.';
  Pf=xhatf.Px;
  xp=xhatp.x.';
  Pp=xhatp.Px;
...
[nx,nu,nv,ny]=size(s);
if nv>0
%    error('SigSys:generic', 'LSS.LSS2LTF: stochastic systems (nv>0) cannot be converted to LTF')
end
...
if isnan(s.fs)
    error('SigSys:generic', 'ss2armax only for discrete time models')
end
[nx,nu,nv,ny]=size(s);
[A,B,C,D,Q,R,S,fs]=getss(s);
a=real(poly(A));
...
if ~isempty(S) & any(S~=0)
   error('SigSys:generic', 'LSS.LSS2SPEC for S~=0 not yet implemented')
end
if nv>ny
   error('SigSys:generic', 'LSS.LSS2SPEC presumes nv<=ny')
end
...
if isempty(opt.f);
    ...
    f=opt.f;
    if ~isnumeric(f), error('SigSys:generic', 'Frequency vector f must be numeric'), end
    f=f(:).';
end
N=length(f);
if nv==0
   if isempty(R) | all(all(R==0))
      error('SigSys:generic', 'LSS.LSS2SPEC is pointless when there is no noise (nv=0 and R=0)')
   else
      for k=1:N
         Phi(k,:,:)=R;
      end
...
if nv==0
   if isempty(R) | all(all(R==0))
      error('SigSys:generic', 'LSS.LSS2COVFUN is pointless when there is no noise (nv=0 and R=0)')
   else
      r(1,:,:)=R;
      for k=2:opt.taumax
         r(k+1,:,:)=zeros(size(R));
...
if size(D,1)~=size(D,2)
    error('SigSys:generic', 'Root locus only for square systems')
end
for j=1:length(K);
...
[nx,nu,nv,ny]=size(s);
if nu~=ny
    error('SigSys:generic', 'EYE works only for square systems')
else
    sys=lss([],[],[],eye(nu),s.fs);
end
end
...
if isempty(s.sysMC) & isempty(s.D) % Models from the prior
    ...
    for k=1:MC;
        ...
        if ny>1;
             %error('SigSys:generic', 'Only MISO systems implemented'),
             C=[C;rand(ny-1,nx)];
             D=[D;rand(ny-1,nu)];
        end
...
if ~isnumeric(MC) | length(MC)>1 | MC<0 | MC~=round(MC)
    error('SigSys:generic', 'LSS.UNCERTAIN: MC must be a positive integer')
elseif MC==0
   su=s;
   return
end
if isa(X,'pdfclass')
   ...
   if N==length(X)
      ...
      error('SigSys:generic', 'LSS.UNCERTAIN: when setting a number of parameters to a PDFCLALSS, the length of X must match')
   end
else
   error('SigSys:generic', 'LSS.UNCERTAIN: X must be a PDFCLALSS object')
end
end
...
if fs>0 % discrete time
   if isnumericscalar(T)
      ...
      if N<2
         error('SigSys:generic', 'LSS.IMPULSE: Simulation time T must be larger than 2/fs')
      end
      t=(0:N-1)'/fs;
   else
      error('SigSys:generic', 'LTF.IMPULSE: T must be either empty or a scalar T>0 for discrete systems')
   end
   if N<2
      error('SigSys:generic', 'LSS.IMPULSE: Simulation time T must be larger than 2/fs')
   end
   ...
   elseif isvector(T) & all(T>=0)
      ...
      error('SigSys:generic', 'LTF.IMPULSE: T must be either empty, a scalar T>0 or a time vector t>=0 for continuous time systems')
   end
    for k=1:length(t);
...
if fs>0 % discrete time
   if isnumericscalar(T)
      if length(T)>1
         error('SigSys:generic', 'LSS.STEP: T cannot be a vector for discrete time systems')
      end
      N=round(T*fs);
      if N>1e3; N=30; end  % Limit response for unstable systems
      if N<2
         error('SigSys:generic', 'LSS.STEP: Simulation time T must be larger than 2/fs')
      end
      t=(0:N-1)'/fs;
   else
      error('SigSys:generic', 'LTF.STEP: T must be either empty or a scalar T>0 for discrete systems')
   end
   u=ones(N,nu);
   [y,x]=ssdlsim(A,B,C,D,u);
else
   ...
   elseif isvector(T) & all(T>=0)
      ...
      error('SigSys:generic', 'LTF.STEP: T must be either empty, a scalar T>0 or a time vector t>=0 for continuous time systems')
   end
    for k=1:length(t);
        E=real(expm([ [A B]*t(k); zeros(nu,nx+nu)]));
...
if size(opt.xi,1)~=nx
    if opt.xi==0
        ...
        error('SigSys:generic', 'Initial state x0 must be either 0 or a row vector of appropriate size')
    end
end
if fs>0
    %-------------------------------
    ...
    if isa(in1,'sig')
        ...
        [N,nudata]=size(u);
        if nudata~=nu
           error('SigSys:generic', 'LSS.SIMULATE: Number of inputs in u not compatible with model')
        end
        if isnan(fs)
           error('SigSys:generic', 'LSS.SIMULATE: discrete time models are simulate only with discrete time signals')
        end
        N=size(u,1);
    ...
    elseif isa(in1,'double')
        u=in1;
        [N,nudata]=size(u);
        %if N<nudata; error('SigSys:generic', 'Input matrix u should have more rows than columns'), end
        if nudata~=nu
           error('SigSys:generic', 'Number of inputs in u not compatible with model')
        end
    else
        error('SigSys:generic', 'Inappropriate argument in1. Should be either input or number of data.')
    end
    t=(0:N-1)'/fs;
    ...
    if nv>0 & nu>0
        [y,x]=ssdlsim(A,[B eye(nx) zeros(nx,ny)],C,[D zeros(ny,nx) eye(ny)],[u v.' e.'],opt.xi);
    elseif nu==0 % No input
        [y,x]=ssdlsim(A,[eye(nx) zeros(nx,ny)],C,[zeros(ny,nx) eye(ny)],[v.' e.'],opt.xi);
    elseif nv==0 % No noise
        [y,x]=ssdlsim(A,B,C,D,u,opt.xi);
    else %??
        error('SigSys:generic', 'LSS.SIMULATE: Either u or v needed as input')
    end
else
  %-------------------------------
  ...
  if nu>0 % in1 gives input signal
    if nv>0
       error('SigSys:generic', 'LSS.SIMULATE: continuous time stochastic IO systems not implemented (both u and v terms in LSS model)')
    end
    if  ~isa(in1,'sig')
      error('SigSys:generic', 'LSS.SIMULATE: For input output systems, second input argument must be a SIG object')
    else
      ...
      [N,nudata]=size(in1.y);
      if nudata~=nu
        error('SigSys:generic', 'LSS.SIMULATE: Number of inputs in SIG object not compatible with model')
      end
      u=in1;
      tu=u.t;
      ...
      for k=1:length(indbreak)-1;           % Simulate one interval at the time
          ...
          if Nk==1; error('SigSys:generic', 'STEP: sampling interval Ts too small'); end
   ...
   %      tk(1)=[];            % Avoid segment boundaries with steps and impulses
          ...
          [ykmat,xkmat]=ssdlsim(A,B,C,D,ukint,xi);
          if  any(find(indbreak(k)==indimp))       % Fix for impulses
      ...
      y=ymat; u=umat; x=xmat;
        ...
        if isnumericscalar(in1)
            ...
            error('SigSys:generic', 'For continuous time noise models, second input argument should be simulation time')
        end
  end
end
...
A=s.A; B=s.B; C=s.C; D=s.D; Q=s.Q; R=s.R; S=s.S; fs=s.fs; nn=s.nn;
nx=nn(1); nu=nn(2); nv=nn(3); ny=nn(4);
texcode=sprintf(['\\begin{eqnarray*}']);
if nx>0
    if fs>0
...
%    s1=lss(0,0,0,s1);
elseif ~isa(s1,'lss')
    error('SigSys:generic', 's1 must be LSS, LTF or a scalar')
end
if isa(s2,'ltf')
    s2=lss(s2);
...
%    s2=lss(0,0,0,s2);
elseif ~isa(s2,'lss')
    error('SigSys:generic', 's2 must be LSS, LTF or a scalar')
end

if isnan(s1.fs) & isnan(s2.fs)   % Continuous time
    fs=NaN;
elseif isnan(s1.fs) | isnan(s2.fs)
    error('SigSys:generic', 'LSS: cannot mix continuous and discrete time models')
elseif s1.fs==s2.fs
    fs=s1.fs;
else
    error('SigSys:generic', 'LSS: Multi-rate LTI operations are not possible')
end
end
...
if ~isnumeric(A) | ~isnumeric(B) | ~isnumeric(C) | ~isnumeric(D)
    error('SigSys:generic', 'A, B, C, and D must be numeric')
end
if ~isempty(D)
   [ny,nu] = size(D);
   [nxB,nuB] = size(B);
...
if isempty(A),
    nx=0; nx2=0;
    if ~isempty(B)
       error('SigSys:generic', 'If A is empty, B must be empty')
    end
    if ~isempty(C)
       error('SigSys:generic', 'If A is empty, C must be empty')
    end
else
    [nx,nx2] = size(A);
end
...
if  nx~=nx2
    error('SigSys:generic', 'A must be square')
end
if nu>0 & nxB~=nx
    error('SigSys:generic', 'Number of rows in B must equal the dimension of A')
end
if nxC~=nx
    error('SigSys:generic', 'Number of columns in C must equal the dimension of A')
end
if nx>0 & nu>0 & nyD~=ny
    % exception for systems with A=B=C=[] and noise systems B=D=[]
    error('SigSys:generic', 'Number of rows in D and C must be the same')
end
if nu>0 & (nuD~=nu)  % exception for systems with A=B=C=[]
    error('SigSys:generic', 'Number of columns in B must be equal to the number of columns in D')
end
Q=zeros(nx,nx);
R=zeros(ny,ny);
S=zeros(nx,ny);
...
elseif length(varargin)==3
    ...
    fs = varargin{3};
    if ~iscov(Q)
        error('SigSys:generic', 'Q must be a valid covariance (positive semidefinite symmetric) matrix')
    end
    if ~iscov(R)
        error('SigSys:generic', 'R must be a valid covariance (positive semidefinite symmetric) matrix')
    end
elseif length(varargin)==4
    ...
    if  ~isnumeric(S)
        error('SigSys:generic', 'S must be numeric')
    end
else
    error('SigSys:generic', 'Incorrect number of arguments')
end
if ~isequal(size(Q),[nx nx])
   error('SigSys:generic', 'LSS: Q must be of size (nx,nx)')
end
if ~isequal(size(R),[ny ny])
    error('SigSys:generic', 'LSS: R must be of size (ny,ny)')
end
if ~isequal(size(S),[nx ny])
    error('SigSys:generic', 'LSS: S must be of size (nx,ny)')
end

if ~isnan(fs) & (~isnumericscalar(fs) | fs<0)
    error('SigSys:generic', 'fs must be a non-negative number or NaN')
end
end
