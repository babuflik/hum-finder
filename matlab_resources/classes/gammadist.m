...
elseif nargin==2
    if ~isnumericscalar(a) | ~isnumericscalar(b) | a<=0 | b<=0 | ...
       ~isreal(a) | ~isreal(b)
        error('SigSys:generic', 'a and b must be positive scalars')
    else
        X.a=a;
        X.b=b;
    end
else
    error('SigSys:generic', 'a and b must be specified')
end
end
...
%   error('SigSys:generic', 'GAMMADIST.POSTERIOR: lh must be either expdist or gammadist')
end
