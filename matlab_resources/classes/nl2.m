...
if nargin<3
   error('SigSys:generic', 'NL2 constructor: three input arguments required')
end
if nargin<4
   fs=NaN;
end
...
if ~isvector(nn) | length(nn)~=5
   error('SigSys:generic', 'NL2 constructor: nn must be a vector of length 5')
end
   ...
   if exist(f,'file')  % m-file
      f=[f,'(t,x,v,u,th)'];
       ...
       try
          ...
          error('SigSys:generic', 'NL2 constructor: a string argument for f must be a valid m-file or a valid inline input argument')
       end
   end
% $$$    try
% $$$       f=inline(f,'t','x','v','u','th');
...
elseif isempty(f) & nx==0
   ...
   error('SigSys:generic', 'NL2 constructor: f must be an inline object, a string, or an m-file')
end
...
elseif isstr(h)
   if exist(h,'file')  % m-file
      ...
      try
         ...
         error('SigSys:generic', 'NL2 constructor: a string argument for h must be a valid m-file or a valid inline input argument')
      end
   end
elseif isempty(h)
   h=[];
else
   error('SigSys:generic', 'NL2 constructor: h must be an inline object, a string, or an m-file')
end
x0=zeros(nx,1);
th=zeros(nth,1);
v=zeros(nv,1);
...
%       fdum=feval(f,0,x0,zeros(nu,1),th);
       ...
       error('SigSys:generic', 'NL2 constructor: sizes in nn not consistent with f')
    end
   if size(fdum,1)~=nx
      disp(['size(f): ',num2str(size(fdum))])
      disp(['size(x,1): ',num2str(nx)])
      error('SigSys:generic', 'NL2 constructor: size of f not consistent with nn')
   end
   if size(fdum,2)~=1
      error('SigSys:generic', 'NL2 constructor: number of columns in f is not one')
   end
   try
       ...
       warning('SigSys:generic', 'NL2 warning: Try to vectorize f for increased speed')
...
if ny>0
   try
      hdum=feval(h,0,1e-5*randn(nx,1),1e-5*randn(ny,1),1e-5*randn(nu,1),1e-5*randn(nth,1));
       ...
       disp(['NL2 error code: ',ME.message])
      error('SigSys:generic', 'NL2 constructor: sizes in nn not consistent with h or error in evaluating h')
   end
   if size(hdum,1)~=ny
      disp(['size(h): ',num2str(size(hdum))])
      disp(['size(y,1): ',num2str(ny)])
      error('SigSys:generic', 'NL2 constructor: size of h not consistent with nn')
   end
   try
      ...
      warning('SigSys:generic', 'NL2 constructor warning: try to vectorize h for increased speed')
   end
end

if ~isnumericscalar(fs)
   error('SigSys:generic', 'NL2 constructor: fs must be a numeric scalar')
end
if fs<0 & ~isnan(fs)
   error('SigSys:generic', 'NL2 constructor: fs must be positive or NaN')
end
m.name=[];
m.desc=[];
...
function m=set.x0(m,x0);
   if ~isempty(x0) & length(x0)~=m.nn(1)
      error('SigSys:generic', 'NL2: cannot change dimension of x0')
   end
   m.x0=x0(:);
end
...
function m=set.th(m,th);
   if ~isempty(th) & length(th)~=m.nn(5)
      error('SigSys:generic', 'NL2: cannot change dimension of th')
   end
   m.th=th(:);
end
...
function m=set.P(m,P);
   if ~isempty(P) & length(P)~=m.nn(5)
      error('SigSys:generic', 'NL2: cannot change dimension of P')
   end
   if ~iscov(P)
      error('SigSys:generic', 'NL2: P is not a valid covariance')
   end
   m.P=P;
end
...
function m=set.I(m,I);
   ...
   elseif  length(I)~=m.nn(1)+m.nn(5)
      error('SigSys:generic', 'NL2: cannot change dimension of I (square nx+nth matrix)')
   end
   if ~iscov(I)
      warning('SigSys:generic', 'NL2 warning: I is not a valid information matrix (symm pos semidefinit)')
   end
   m.I=I;
end
...
function m=set.pe(m,pe);
   ...
   elseif isa(pe,'pdfclass')
       ...
       if length(pe)~=m.nn(4);  % length(e)=ny
          error('SigSys:generic', 'NL2 fieldwrite: length of symbolic pe must equal ny')
       end
   elseif iscov(pe)
       if size(pe,1)~=m.nn(4);  % length(e)=ny
          error('SigSys:generic', 'NL2 fieldwrite: size of covariance matrix pe must equal ny')
       end
       m.pe=ndist(zeros(m.nn(4),1),pe);
   else
      error('SigSys:generic', 'NL2 fieldwrite: pe must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.pv(m,pv);
   ...
   elseif isa(pv,'pdfclass')
       ...
       if length(pv)~=m.nn(2);  % length(v)=nv
          error('SigSys:generic', 'NL2 fieldwrite: length of symbolic pv must equal nv')
       end
   elseif iscov(pv)
       if size(pv,1)~=m.nn(2);
          error('SigSys:generic', 'NL2 fieldwrite: size of covariance matrix pv must equal nv')
       end
       m.pv=ndist(zeros(m.nn(2),1),pv);
   else
      error('SigSys:generic', 'NL2 fieldwrite: pv must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.px0(m,px0);
   ...
   elseif isa(px0,'pdfclass')
       ...
       if length(px0)~=m.nn(1);  % length(x0)=nx
          error('SigSys:generic', 'NL2 fieldwrite: length of symbolic px0 must equal nx')
       end
   elseif iscov(px0)
       if size(px0,1)~=m.nn(1);
          error('SigSys:generic', 'NL2 fieldwrite: size of covariance matrix px0 must equal nx')
       end
       m.px0=ndist(zeros(m.nn(1),1),px0);
   else
      error('SigSys:generic', 'NL2 fieldwrite: px0 must be either a symbolic distribution or a covariance matrix')
   end
end
...
function m=set.fs(m,fs);
   if isnumericscalar(fs) | fs<0
      ...
      error('SigSys:generic', 'NL2 fieldwrite: fs must be a positive scalar or NaN')
   end
end
...
function m=set.xlabel(m,xlabel);
   ...
   elseif isempty(xlabel)
      ...
      error('SigSys:generic', ['NL2 fieldwrite: Field xlabel must be a cell of length nx, [], or string if nx=1'])
   end
end
...
function m=set.ylabel(m,ylabel);
   ...
   elseif isempty(ylabel)
      ...
      error('SigSys:generic', ['NL2 fieldwrite: Field ylabel must be a cell of length ny, [], or string if ny=1'])
   end
end
...
function m=set.ulabel(m,ulabel);
   ...
   elseif isempty(ulabel)
      ...
      error('SigSys:generic', ['NL2 fieldwrite: Field ulabel must be a cell of length nu, [], or string if nu=1'])
   end
end
...
function m=set.thlabel(m,thlabel);
   ...
   elseif isempty(thlabel)
      ...
      error('SigSys:generic', ['NL2 fieldwrite: Field thlabel must be a cell of length nth, [], or string if nth=1'])
   end
end
...
function m=set.tlabel(m,tlabel);
   if  isstr(tlabel)
      ...
      error('SigSys:generic', ['NL2 fieldwrite: Field tlabel must be a string'])
   end
end
...
if nargin<3
   error('SigSys:generic', 'NL2.ARRAYREAD: both input indeces in s(j,i) are required')
end
nv=m.nn(2);
nu=m.nn(3);
ny=m.nn(4);
if isstr(i)
    if strcmp(i,':')
       ...
       error('SigSys:generic', 'NL2.ARRARYREAD: Inputs must be a vector or :')
    end
end
if isstr(j)
    if strcmp(j,':')
       ...
       error('SigSys:generic', 'NL2.ARRAYREAD: Inputs must be a vector or :')
    end
end
if any(i>nu)
     error('SigSys:generic', 'NL2.ARRAYREAD: Output (row) index larger than the number of outputs')
end
if any(j>ny)
     error('SigSys:generic', 'NL2.ARRAYREAD: Input (column) index larger than the number of inputs')
end
sysf=m.f;
sysh=m.h;
sysnn=m.nn;
...
if length(j)<ny % Delete outputs
   if ~isa(m.h,'inline')
      error('SigSys:generic', 'NL2.ARRAYREAD: Removing output is not possible for m-file definition of h')
   end
   I=eye(ny);
...
if isnan(mc.fs)
   ...
   ind2=findstr(fc,';');
   ind=[ind1(1) ind2];
   if length(ind)~=nx, error('SigSys:generic', 'NL2.C2D string interpretation problems'), end
   ind=[ind length(fc)];
   fd=fc(1:ind(1));
   for k=1:nx
   ...
   error('SigSys:generic', 'NL2.C2D cannot change sampling interval of discrete time models')
end
end
...
elseif isempty(m.h)
   ...
   error('SigSys:generic', 'nl2.addsensor: cannot mix mfiles and inline representations of h with this function')
end
ms.fs=m.fs;
ms.pv=m.pv;
ms.x0=m.x0;
...
elseif isa(m.h,'inline')
   ...
   error('SigSys:generic', 'nl2.removesensor: cannot mix mfiles and inline representations of h with this function')
end
end
...
if isnan(m.fs)
    error('SigSys:generic', 'NL2.EKF: only discrete time models')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL2.EKF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL2.EKF: Number of inputs nu must be the same in model and signal')
end
...
if isempty(m.pv)
   warning('SigSys:generic', 'NL2.EKF warning: m.pv is not defined, using v=0')
end
if isempty(m.pe)
   warning('SigSys:generic', 'NL2.EKF warning: m.pe is not defined, using e=0')
end
if isempty(opt.P0) | isnan(opt.P0)
   warning('SigSys:generic', 'NL2.EKF warning: px0 not defined, using a default value instead')
   P0=1e3*eye(nx);
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL2.EKF: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL2.EKF: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nv & size(opt.Q,2)==nv
   ...
   error('SigSys:generic', 'NL2.EKF: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
if isempty(Q)
   Q=0;
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL2.EKF: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL2.EKF: Model must have a non-empty measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL2.EKF: k must be a scalar k>=0')
end
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL2.EKF: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL2.FIM: only discrete time models')
end
nx=m.nn(1);
nv=m.nn(2);
nu=m.nn(3);
...
if isa(z,'sig')
    if isempty(z.x)
        error('SigSys:generic', 'NL2.FIM: the input SIG object z must have a state x')
    end
    x=z.x;
    t=z.t;
    [N,nyz,nuz]=size(z);
    ...
    if nx~=nxdata
       if N==nx
          ...
          N=1;
           ...
           error('SigSys:generic', ['NL2.FIM: the input state vector z must must have ',num2str(nx),' elements'])
       end
    end
end

if N>1
    error('SigSys:generic', 'NL2.FIM: FIM is only evaluated for one state at the time')
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL2.FIM: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL2.FIM: Model must have a non-empty measurement noise covariance R')
end
...
if nargin<2
    error('SigSys:generic', 'NL2.PD: at least two input arguments required')
end
if nargin<3
   pfa=0.01;
end
if ~isnumericscalar(pfa) | pfa<=0 | pfa>=1
    error('SigSys:generic', 'NL2.PD: 0<pfa<1')
end

if isnan(m.fs)
    error('SigSys:generic', 'NL2.PD: only discrete time models')
end
nx=m.nn(1);
nv=m.nn(2);
nu=m.nn(3);
...
if isa(z,'sig')
    if isempty(z.x)
        error('SigSys:generic', 'NL2.PD: the input SIG object z must have a state x')
    end
    x=z.x;
    t=z.t;
    [N,nyz,nuz]=size(z);
    ...
    if nx~=nxdata
       if N==nx
          ...
          N=1;
           ...
           error('SigSys:generic', ['NL2.PD: the input state vector z must must have ',num2str(nx),' elements'])
       end
    end
end

if N>1
    error('SigSys:generic', 'NL2.PD: PD is only evaluated for one state at the time')
end
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL2.CRLB: only discrete time models')
end
if isempty(z.x)
    error('SigSys:generic', 'NL2.CRLB: the input SIG object z must have a state x')
end
...
[N,nyz,nuz]=size(z);
%if nyz~=ny
%   error('SigSys:generic', 'NL2.CRLB: Number of outputs ny must be the same in model and signal')
%end
%if nuz~=nu
%   error('SigSys:generic', 'NL2.CRLB: Number of inputs nu must be the same in model and signal')
%end
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL2.CRLB: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL2.CRLB: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nv & size(opt.Q,2)==nv %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   error('SigSys:generic', 'NL2.CRLB: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL2.CRLB: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL2.CRLB: Model must have a non-empty measurement noise covariance R')
end
if all(diag(R)==0)
   error('SigSys:generic', 'NL2.CRLB: Model must have a positive definite measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL2.CRLB: k must be a scalar k>=0')
end
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL2.CRLB: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL2.NL2TF: only discrete time models')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL2.NL2TF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL2.NL2TF: Number of inputs nu must be the same in model and signal')
end
...
if isempty(m.pv)
   warning('SigSys:generic', 'NL2.NL2TF warning: m.pv is not defined, using v=0')
end
if isempty(m.pe)
   warning('SigSys:generic', 'NL2.NL2TF warning: m.pe is not defined, using e=0')
end
if isempty(opt.P0)  | isnan(opt.P0)
    % Use P0 from m.px0
    warning('SigSys:generic', 'NL2.NL2TF warning: px0 not defined, using a default value instead')
    P0=1e3*eye(nx);
elseif isnumericscalar(opt.P0)
    P0=opt.P0*eye(nx);
elseif iscov(opt.P0)
...
elseif isa(opt.P0,'pdfclass')
   ...
   error('SigSys:generic', 'NL2.NL2TF: P0 must be empty, a scalar, a symmetric (nx,nx) matrix or a PDFCLASS object')
end
...
elseif isnumeric(opt.x0) & size(opt.x0,1)==nx & size(opt.x0,2)==1
   ...
   error('SigSys:generic', 'NL2.NL2TF: x0 must be empty or a (nx,1) vector')
end
...
elseif iscov(opt.Q)  & size(opt.Q,1)==nx & size(opt.Q,2)==nx
   ...
   error('SigSys:generic', 'NL2.NL2TF: Q must be empty, a scalar, or a covariance (nx,nx) matrix')
end
if isempty(Q)
   Q=0;
end
...
elseif iscov(opt.R) & size(opt.R,1)==ny & size(opt.R,2)==ny
   ...
   error('SigSys:generic', 'NL2.NL2TF: Value of ''R'' must be empty, a scalar, or a covariance (ny,ny) matrix')
end
if isempty(R)
   error('SigSys:generic', 'NL2.NL2TF: Model must have a non-empty measurement noise covariance R')
end

if isnumericscalar(opt.k) & opt.k>=0
   %
else
   error('SigSys:generic', 'NL2.NL2TF: k must be a scalar k>=0')
end
...
elseif opt.k==1
   ...
   error('SigSys:generic', 'NL2.NL2TF: k must be either 0 or 1')
end
x.fs=z.fs;
try
  x.xlabel=m.xlabel;
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL2.PF: only discrete time models')
end

if ~isnumericscalar(opt.k) | opt.k<0 | opt.k>1
   error('SigSys:generic', 'NL2.PF: k must be 0 or 1')
end
if ~isnumericscalar(opt.Np) | opt.Np<1
   error('SigSys:generic', 'NL2.PF: Np must a positive scalar')
end

if isempty(opt.ind)
   ind=':';
elseif ~isnumeric(opt.ind)
   error('SigSys:generic', 'NL2.PF: value for argument animate must be numeric')
else
   ind=opt.ind;
end
if ~isnumeric(opt.delay) | opt.delay<0
   error('SigSys:generic', 'NL2.PF: delay must be numeric and positive')
end
...
Np=opt.Np;
if nyz~=ny
   error('SigSys:generic', 'NL2.PF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL2.PF: Number of inputs nu must be the same in model and signal')
end
...
for k=1:N;
   ...
   if strcmp(opt.animate,'on')
      ...
      elseif strcmp(opt.type,'xplot2')
         ...
         if length(ind)~=2
            error('SigSys:generic', 'NL2.PF: ind must have length 2 in xplot2')
         end
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('SigSys:generic', 'NL2.PMF: only discrete time models')
end

if ~isnumericscalar(opt.k) | opt.k<0 | opt.k>1
   error('SigSys:generic', 'NL2.PMF: k must be 0 or 1')
end
if ~isnumericscalar(opt.Np) | opt.Np<1
   error('SigSys:generic', 'NL2.PMF: Np must a positive scalar')
end

if isempty(opt.ind)
   ind=':';
elseif ~isnumeric(opt.ind)
   error('SigSys:generic', 'NL2.PMF: value for argument animate must be numeric')
else
   ind=opt.ind;
end
if ~isnumeric(opt.delay) | opt.delay<0
   error('SigSys:generic', 'NL2.PMF: delay must be numeric and positive')
end
...
[N,nyz,nuz]=size(z);
if nyz~=ny
   error('SigSys:generic', 'NL2.PMF: Number of outputs ny must be the same in model and signal')
end
if nuz~=nu
   error('SigSys:generic', 'NL2.PMF: Number of inputs nu must be the same in model and signal')
end
Np=opt.Np;
if nx==1
...
elseif nx==2
  ...
  pv=reshape(pv,N1,N1);
   ...
   error('SigSys:generic', 'NL2.PMF: nx must be less than or equal to 2 in the PMF')
end
...
if nargin<2
   error('SigSys:generic', 'NL2 estimate: two input arguments required')
elseif ~isa(m,'nl2')
   error('SigSys:generic', 'NL2 estimate: input argument 1 must be an NL2 object')
elseif ~isa(z,'sig')
   if iscell(z)
      for kk=1:length(z)
         if ~isa(z{kk},'sig')
            error('SigSys:generic', 'NL2 estimate: input argument 2 must be (an array of) SIG object')
         end
      end
   else
      error('SigSys:generic', 'NL2 estimate: input argument 2 must be a SIG object')
   end
end
...
if nargin<2
   error('SigSys:generic', 'NL2.simulate: two input arguments required')
end
opt=struct('MC',30);
opt=optset(opt,varargin);
...
if isa(in1,'sig')
   ...
   if ~isnan(in1.fs) & in1.fs~=m.fs
      error('SigSys:generic', 'NL2.simulate: m and z must have the same sampling frequency')
   end
   if size(u,1) ~= nu
      error('SigSys:generic', 'NL2.simulate: input dimension of m must match output dimension of z')
   end
elseif isvector(in1)
   ...
   elseif length(in1)==2
      ...
      if any(abs(diff(t)-Ts)>1e-12)
         error('SigSys:generic', 'NL2.simulate: second input as a time vector must have fs consistent with the model')
      end
      if m.fs~=1/Ts;
   ...
   error('SigSys:generic', 'NL2.simulate: second input argument must be a vector or SIG object')
end
N=length(t);
t=t(:);
...
if nx>0
   ...
   if size(y,2)~=ny | size(ny,1)~=length(t) % loop over time
      ...
      elseif size(y,2)==ny & size(y,1)==length(t)
         ...
         error('SigSys:generic', 'NL2.SIMULATE: incorrect output from h')
      end
end
% $$$ if isa(m.pe,'pdfclass')
...
if nargin<2
   error('SigSys:generic', 'NL2 nl22ss: two input arguments required')
elseif ~isa(m,'nl2')
   error('SigSys:generic', 'NL2 nl22ss: first input argument must be an NL2 object')
elseif ~isa(z,'sig')
   error('SigSys:generic', 'NL2 nl22ss: second input argument must be a SIG object')
elseif size(z,1)>1
   error('SigSys:generic', 'NL2 nl22ss: SIG object z has to be one sample, not a signal')
elseif  isempty(z.x)
   error('SigSys:generic', 'NL2 nl22ss: SIG object z must contain a state z.x')
end
...
if size(z.x,2)~=nx
   error('SigSys:generic', 'NL2 nl22ss: SIG object state z.x does not match the state dimension in m')
elseif nu>0 & isempty(z.u)
   error('SigSys:generic', 'NL2 nl22ss: model m has an input, but the SIG object z.u is empty')
elseif nu~=size(z.u)
   error('SigSys:generic', 'NL2 nl22ss: model m input has not the same dimension as the SIG object z.u')
end
...
if nargin<3
   error('SigSys:generic', 'NL2 nl2numgrad: at least three input arguments required')
elseif ~isa(m,'nl2')
   error('SigSys:generic', 'NL2 nl2numgrad: first input argument must be an NL2 object')
elseif ~isa(z,'sig')
   error('SigSys:generic', 'NL2 nl2numgrad: second input argument must be a SIG object')
elseif ~isstr(derstr)
   error('SigSys:generic', 'NL2 nl2numgrad: third input argument must be a string')
end
...
if strcmpi(derstr,'dfdx') | strcmpi(derstr,'dhdx')
    ...
    if sum(mask)~=length(find(mask)),
       error('SigSys:generic', 'NL2 nl2numgrad: mask must be binary'),
    end
    n=sum(mask);
...
elseif strcmpi(derstr,'dxdth') | strcmpi(derstr,'dydth')
    ...
    for j=1:n
     ...
     error('SigSys:generic', 'NL2 nl2numgrad: unknown option for der')
end
end
